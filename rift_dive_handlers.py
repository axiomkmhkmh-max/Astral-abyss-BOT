...
ROOM_LABELS = {
    ...
    "shrine":   "🕍 معبدِ انتخاب",
    "rest":     "🕯 اتاقِ استراحت",
    "max_depth": "🏁 عمقِ نهایی — باید کش‌اوت کنی",
}
...
def _status_text(player: dict) -> str:
    ...
    text = (
        ...
        f"{' | روحِ×' + str(len(run['pending_materials'])) if run.get('pending_materials') else ''}\n"
        f"🏦 بانک‌شده: {run['banked_zen']:,} Zen | 🔹{run['banked_shards']} | 📦{len(run['banked_items'])}"
        f"{' | روحِ×' + str(len(run['banked_materials'])) if run.get('banked_materials') else ''}\n"
    )
    if run.get("blessings"):
        from collections import Counter
        counts = Counter(run["blessings"])
        parts = []
        for b, n in counts.items():
            opt = rd.SHRINE_OPTIONS.get(b)
            if not opt:
                continue
            parts.append(f"{opt['name']}" + (f" ×{n}" if n > 1 else ""))
        text += "✨ " + " | ".join(parts) + "\n"
    if run.get("log"):
        text += "\n" + "\n".join(run["log"])
    return text
...
async def cb_rift_next(cb: CallbackQuery):
    ...
    if result["dead"]:
        ...
        text = (
            ...
            f"{' | 🔹تکه‌ی روح×' + str(summary['soul_shards_gained']) if summary['soul_shards_gained'] else ''}\n"
            f"❌ ازدست‌رفته: {summary['lost_pending_zen']:,} Zen | 🔹{summary['lost_pending_shards']}"
            f"{' | 🔹تکه‌ی روح×' + str(summary['lost_pending_materials']) if summary['lost_pending_materials'] else ''}\n"
...
async def cb_rift_cashout(cb: CallbackQuery):
    ...
    text = (
        ...
        f"{' | 🔹+' + str(summary['soul_shards_gained']) + ' تکه‌ی روح' if summary['soul_shards_gained'] else ''}"
    )
...
async def cmd_rift_reset(msg: Message):
    """🩹 شیرِ اضطراری — اگه به هر دلیلی رانِ فعال گیر کرد و نه دروازه‌ی
    خروج و نه کش‌اوت جواب داد، این دستور اول تلاش می‌کنه عینِ کش‌اوتِ عادی
    همه‌چیز (banked + pending) رو امن بانک کنه؛ فقط اگه همون هم به هر
    دلیلی خطا داد، به‌عنوانِ آخرین راه، رانِ گیرکرده رو بدونِ پاداش پاک
    می‌کنه تا بازیکن برای همیشه گیر نکنه."""
    uid = msg.from_user.id
    player = await aget_player(uid)
    if not player or not player.get("rift_run"):
        await msg.answer("❌ رانِ فعالی نداری.")
        return
    try:
        summary = rd.finalize_run(player, died=False)
        await asave_player(uid, player)
        await msg.answer(
            "✅ رانِ گیرکرده با موفقیت بسته شد و همه‌چیز بانک شد.\n"
            f"عمقِ رسیده: {summary['depth_reached']} | 💰 +{summary['zen_gain']:,} Zen | 🔹+{summary['shard_gain']}"
        )
    except Exception as e:
        log_sync(f"🆘 rift_reset fallback (finalize_run خطا داد) uid={uid}: {type(e).__name__}: {e}", "RIFT")
        player["rift_run"] = None
        await asave_player(uid, player)
        await msg.answer("🆘 رانِ گیرکرده پاک شد (پاداشِ این ران متأسفانه ذخیره نشد). می‌تونی از اول شروع کنی: /riftdive")


def register_rift_dive_handlers(dp, bot):
    dp.message.register(cmd_riftdive, Command("riftdive"))
    dp.message.register(cmd_rift_reset, Command("riftreset"))
