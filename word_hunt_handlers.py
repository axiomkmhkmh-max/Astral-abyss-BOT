...
_nudge_cooldown: dict[int, float] = {}   # uid -> آخرین باری که به یه غریبه نادج زدیم
NUDGE_COOLDOWN = 5 * 60
...
async def handle_word_hunt(message: Message):
    ...
    if not player:
        # 🆕 به‌جای سکوتِ کامل، یه نادجِ کوتاه بده — دقیقاً همون لحظه‌ای
        # که یه عضوِ تازه‌وارد کنجکاو می‌شه ببینه این کلمه چیه. سکوتِ
        # کامل یعنی از دست دادنِ لحظه‌ی تبدیل به بازیکنِ جدید.
        last_nudge = _nudge_cooldown.get(uid, 0)
        if now - last_nudge > NUDGE_COOLDOWN:
            _nudge_cooldown[uid] = now
            try:
                await message.reply(
                    "👀 انگار بلدی! ولی اول باید کاراکترت رو بسازی.\n"
                    "به ربات پیام بده و /start رو بزن، بعد برگرد همینجا و دوباره امتحان کن."
                )
            except Exception:
                pass
        return
    ...
    # ─── بونوسِ همدمِ محبوب (companion_gacha_handlers) ───────────
    from companion_gacha_handlers import get_favorite_zen_bonus
    companion_zen_bonus = get_favorite_zen_bonus(player)
    ...
    zen_gain = int(zen_gain * streak_mult * synergy_mult * (1 + guild_zen_bonus + companion_zen_bonus) * pulse_zen_mult)
