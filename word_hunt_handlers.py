...
async def handle_word_hunt(message: Message):
    ...
    guild_xp_bonus = af.cap_bonus(get_perk(player, "xp_gain_pct") + get_war_xp_buff(player))

    # ─── World Pulse (ضربانِ زنده‌ی جهان) ───────────────────────
    pulse_xp_mult = wp.pulse_value("xp_mult")
    pulse_zen_mult = wp.pulse_value("zen_mult")
    active_pulse = wp.get_active_pulse()
