...
def is_resting(player: dict) -> bool:
    """اگه دوره‌ی استراحتِ ست‌شده کامل تموم شده باشه، خستگی رو صفر می‌کنه —
    قبلاً فقط وقتی صفر می‌شد که دقیقاً وسطِ بازه‌ی استراحت یه نبردِ جدید
    شروع می‌شد، پس اگه کسی فقط صبر می‌کرد پیامِ قفل تکرار می‌شد."""
...
async def cmd_rebirth(msg: Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="🔄 آره، Rebirth کن!", callback_data="rebirth_go"),
        InlineKeyboardButton(text="❌ بی‌خیال", callback_data="rebirth_cancel"),
    ]])
    await msg.answer(
...
async def cb_atk_help(cb: CallbackQuery):
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 برگشت به حمله", callback_data="atk:menu")]
    ])
...
async def _render_attack_panel(uid: int, player: dict) -> tuple[str, InlineKeyboardMarkup]:
    ...
    buttons.append([InlineKeyboardButton(text="❓ فرقِ حملات چیه؟", callback_data="atk_help")])
    buttons.append([InlineKeyboardButton(text="📊 آمار مبارزه", callback_data="atk_stats")])
    buttons.append([InlineKeyboardButton(text="📜 کوئست حمله", callback_data="hunt:panel")])

    # 🩹 دسترسیِ سریع به پنلِ فعالِ کلاس (طلسم/مزدور/نورِ مقدس/دخمه) —
    # قبلاً این دکمه فقط تو تلگرام بود؛ روی گپ کلاً چنین ورودی‌ای نبود.
    ...
    if _cls_info:
        buttons.append([InlineKeyboardButton(
            ...
            callback_data="class_panel",
        )])

    if fight:
        enemy = fight
        tier_emoji = {"common":"⚪","rare":"🔵","epic":"🟣","legendary":"🟡"}.get(enemy.get("tier","common"),"⚪")
        buttons.append([InlineKeyboardButton(text="🏃 فرار از نبرد", callback_data="atk_flee")])
        st_txt = ""
...
async def cb_atk_select(cb: CallbackQuery):
    ...
    buttons.append([InlineKeyboardButton(text="❌ لغو", callback_data="atk:cancel")])
...
async def resolve_hit(cb: CallbackQuery, player: dict, atk_type: str):
    ...
    # 🐛 فیکس: همون مشکلِ combat_handlers.py — دشمنِ مرده دیگه نمی‌تونه
    # ضربه‌ی مرگبار بزنه.
    deadly = maybe_deadly_blow(player, enemy) if (player.get("hp", 100) > 0 and enemy.get("hp", 1) > 0) else None
    if deadly:
    ...
    if killed:
        ...
        # ─── باگ‌فیکس (هم‌سنگِ combat_handlers.py): مسیرِ «حمله» ست‌ها/
        # آیتم‌ها/همراه/کوئست‌لاینِ شکار رو تو Zen/XP حساب نمی‌کرد.
        setb = {}
        try:
        ...
        if leveled and player.get("mentee_of"):
            ...
            if bond_lines:
                ...
                try:
                    # نکته‌ی گپ: player["mentee_of"] یه uid داخلیه (منفی)، ولی
                    # bot.send_message به chat_id واقعیِ گپ (مثبت) نیاز داره.
    ...
    if killed:
        kb = InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="⚔️ دشمن بعدی",   callback_data=f"atk:{atk_type}"),
            InlineKeyboardButton(text="🔙 منوی حمله",   callback_data="atk:menu"),
        ]])
    elif respawned_map:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="⚔️ ادامه نبرد", callback_data=f"atkc:{atk_type}"),
            InlineKeyboardButton(text="🏃 فرار",        callback_data="atk_flee"),
        ]])
    ...
    if tutorial_graduated:
        # نکته‌ی گپ: main_kb/_is_group_chat مخصوصِ ریپلای‌کیبوردِ تلگرامه و روی
        # گپ معنی نداره (گپ گروه رو این‌جوری تشخیص نمی‌ده) — به‌جاش منوی
        # اصلیِ گپ (همون چیزی که gap_core_handlers می‌سازه) رو نشون می‌دیم.
        from gap_core_handlers import main_menu_kb
        await cb.message.answer("🔓 پنلِ کامل باز شد — از دکمه‌های زیر استفاده کن:", reply_markup=main_menu_kb())
    elif tutorial_loot_step:
        # نکته‌ی گپ: onboarding.tutorial_kb_loot() یه ReplyKeyboardMarkup تلگرامیه
        # که رو گپ معنی نداره؛ سیستمِ لوت الان کامل رو گپ کار می‌کنه
        # (gap_loot_handlers.py → cmd_loot، commands=["loot"])، فقط از طریقِ
        # ReplyKeyboard در دسترس نیست. پس این‌جا راهنمایی می‌دیم دستورِ
        # /loot رو بزنن — همون‌جا onboarding.on_loot_visited صدا زده می‌شه و
        # تیوتوریال کامل می‌شه. (قبلاً این پیام می‌گفت لوت «هنوز پورت نشده»،
        # که یه بن‌بستِ واقعی بود: پلیرهایی که اولین کیلشون تو گپ بود، هیچ‌وقت
        # راهی برای رسیدن به قدمِ لوت نداشتن و برای همیشه رو کیبوردِ
        # دوتا-دکمه‌ای (حمله/وضعیت) گیر می‌کردن.)
        await cb.message.answer("🗺 **قدمِ سوم:** دستورِ /loot رو بزن — برو تو نقشه، یه لوکیشن بگرد تا تیوتوریال کامل شه!")
...
async def cmd_wall(msg: Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="⚔️ شروع چالش", callback_data=f"wallch:{lvl}")
    ]])
    await msg.answer(
...
async def cb_wall_challenge(cb: CallbackQuery):
    ...
    from characters import ALL_CHARACTERS
    char = next((c for c in ALL_CHARACTERS if c["name"] == player.get("character")), None)
    p_dmg = max(5, int((char["base_dmg"] if char else 10) + player.get("level", 1) * 1.5))
...
# ─── Register (نسخه‌ی گپ) ──────────────────────────────────────
#  نکته‌ی معماری: aiogram با ترکیبِ فیلترها یه استثنا می‌ساخت
#  (F.data.startswith("atk:") & ~F.data.in_({...})) تا atk:locked/
#  atk:cancel/atk:menu رو از هندلرِ عمومیِ atk: جدا کنه. GapDispatcher
#  همچین ترکیب‌فیلتری نداره، ولی چون تو _dispatch_message/_dispatch
#  قوانین به «ترتیبِ رجیستر» چک می‌شن و اولین تطبیق برنده‌ست، همون
#  نتیجه رو با رجیسترکردنِ match‌های دقیق (atk:locked/cancel/menu)
#  *قبل از* match عمومیِ startswith="atk:" به‌دست میاریم.
def register_gap_combat_handlers(dp: GapDispatcher):
    dp.register_message(cmd_attack, commands=["attack"])
    dp.register_message(cmd_combat_stats, commands=["combatstats"])

    # 🆕 میان‌بُرهای کلمه‌ای — فقط توی گروهِ گپ کار می‌کنن (تو پیوی
    # کاربر رو به کامندِ اسلشیِ خودش ارجاع می‌دن). نگاه کن به
    # gap_group_gate.py برای توضیحِ کامل.
    from gap_group_gate import group_word
    dp.register_message(group_word(cmd_attack, "/attack"), text="حمله")
    dp.register_message(group_word(cmd_combat_stats, "/combatstats"), text="وضعیت")
    # نکته: /heal و کال‌بک‌های "heal:*" اینجا هم مثل نسخه‌ی تلگرام ثبت
    # نمی‌شن چون قراره gap_team_handlers.py (وقتی پورت شد) نسخه‌ی کامل‌تر
    # (هیل هم‌تیمی + آیتم) رو بده. cmd_heal/cb_heal برای ارجاع می‌مونن.
    dp.register_message(cmd_event,  commands=["event"])
    dp.register_message(cmd_quests, commands=["quests"])
    dp.register_message(cmd_wall,   commands=["wall"])
    dp.register_message(cmd_rest,   commands=["rest"])
    dp.register_message(cmd_rebirth, commands=["rebirth"])
    dp.register_callback(cb_rebirth_go, data="rebirth_go")
    dp.register_callback(cb_rebirth_cancel, data="rebirth_cancel")
    dp.register_callback(cb_wall_challenge, data_startswith="wallch:")

    # ─── ترتیب مهمه: match‌های دقیق قبل از match عمومیِ "atk:" ───
    dp.register_callback(cb_atk_locked, data="atk:locked")
    dp.register_callback(cb_atk_cancel, data="atk:cancel")
    dp.register_callback(cb_atk_menu,   data="atk:menu")
    dp.register_callback(cb_atk_select, data_startswith="atk:")
    dp.register_callback(cb_atk_enemy,   data_startswith="atk_enemy:")
    dp.register_callback(cb_atk_continue, data_startswith="atkc:")
    dp.register_callback(cb_atk_flee,    data="atk_flee")
    dp.register_callback(cb_set_stance,  data_startswith="stance:")
    dp.register_callback(cb_atk_help,    data="atk_help")
    dp.register_callback(cb_atk_stats,   data="atk_stats")
