...
async def cb_pet_hatch(cb: CallbackQuery):
    uid = cb.from_user.id
    # 🔒 باگ‌فیکس: قبلاً این هندلر بدونِ player_lock بود. اگه بازیکن دوبار
    # پشتِ‌سرِهم رو دکمه‌ی «خریدِ تخم» می‌زد (دابل‌تپِ موبایل خیلی رایجه)،
    # دو تا کالبکِ هم‌زمان هر دو با یه نسخه‌ی قدیمیِ player شروع می‌کردن،
    # هر دو یه پت اضافه می‌کردن، ولی چون save_player آخری کاملِ دیتای
    # خودشو (که پتِ اونیکی توش نبود) می‌نوشت، یکی از دو پتِ تازه‌هچ‌شده
    # (یا حتی هر دو) از لیستِ نهایی گم می‌شد — دقیقاً همون باگِ «دوتا
    # حماسی گرفتم ولی هیچ‌کدوم تو لیستم نبود». player_lock(uid) این
    # بلاک رو اتمیک می‌کنه تا کالبکِ دوم منتظرِ تمومِ‌شدنِ اولی بمونه.
