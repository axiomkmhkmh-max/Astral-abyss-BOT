...
# طبق درخواستِ حسین: پلیرا بدونِ دونستنِ یوزرنیم بتونن مغازه‌ی همدیگه رو
# ببینن و باهم خرید و فروش کنن. قبلاً فقط با /visit @username می‌شد.
def list_active_shops(skip: int = 0, limit: int = 8) -> list:
    """لیستِ مغازه‌های فعال (حداقل ۱ کالا)، مرتب‌شده بر اساسِ کل فروش —
    مغازه‌های پرکارتر بالاترن."""
    from database import players_col
    cur = players_col().find(
        {"shop.listings.0": {"$exists": True}},
        {"_id": 1, "name": 1, "shop.name": 1, "shop.listings": 1,
         "shop.reputation": 1, "shop.total_sales": 1, "shop.tier": 1},
    ).sort("shop.total_sales", -1).skip(skip).limit(limit)
    return list(cur)


def count_active_shops() -> int:
    from database import players_col
    return players_col().count_documents({"shop.listings.0": {"$exists": True}})
