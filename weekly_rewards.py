...
async def _run_once(bot):
    ...
    if top_overall:
        ...
        try:
            ...
            await broadcast_achievement(
                f"👑 **{p.get('name','یه بازیکن')}** قهرمانِ هفته شد! "
                f"(سطح {p.get('level',1)}، {p['weekly_champion_count']}اُمین بار) 🎉"
            )
        except Exception:
            pass
    ...
    # ─── ۳) چالشِ رقابتیِ هفتگیِ چرخشی (weekly_challenge.py) ────────
    try:
        from weekly_challenge import rotate_and_reward
        await rotate_and_reward(bot, players)
    except Exception:
        pass
