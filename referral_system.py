...
#     ref_user_<uid>           → از رو لینکِ دعوتِ شخصیِ یه بازیکن (/invite)
#
...
import time
import re
from mongo_shim import Collection

from database import get_db

BOT_USERNAME = "ASTRALABYSS_BOT"
BOT_LINK = f"https://t.me/{BOT_USERNAME}"

_PAYLOAD_RE = re.compile(r"^ref_(group|card|duel|user)_(-?\d+)$")
...
def user_ref_link(uid: int) -> str:
    return f"{BOT_LINK}?start=ref_user_{uid}"
...
def track_referral(payload: str | None, new_uid: int) -> dict | None:
    ...
    # ضدِ خودارجاعی: کسی نمی‌تونه با ساختنِ لینکی به آیدیِ خودش رفرال بگیره
    if parsed["kind"] != "group" and parsed["value"] == new_uid:
        return None
