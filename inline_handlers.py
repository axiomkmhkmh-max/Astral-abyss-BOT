...
# ------------------------------------------------------------
#  با تایپِ  @ASTRALABYSS_BOT  تویِ ANY چت (حتی چتی که ربات توش
...
from referral_system import card_ref_link, duel_ref_link, user_ref_link, BOT_LINK
...
def _invite_result(link: str = BOT_LINK) -> InlineQueryResultArticle:
    text = (
    ...
    return InlineQueryResultArticle(
        ...
        input_message_content=InputTextMessageContent(message_text=text),
        reply_markup=_play_kb(link, "🌑 شروع ماجراجویی"),
...
async def handle_inline_query(iq: InlineQuery):
    ...
    # لینکِ دعوتِ شخصی (اگه بازیکن باشه) تا هر شیر، به حسابِ خودش ثبت بشه — viral_loop.py
    results.append(_invite_result(user_ref_link(uid) if player and player.get("character") else BOT_LINK))
