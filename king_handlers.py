...
#  نقشه. یه‌بار در روز آدیانسِ واقعی (Zen + شانسِ آیتم/گیر + شانسِ
#  لطفِ ویژه) می‌ده؛ بعدش فقط فلیوره، تا فارم‌کردن ممکن نباشه.
#
#  علاوه بر آدیانسِ روزانه، بازیکن می‌تونه با «🎁 تقدیمِ پیشکش» با
#  Zen اعتبارش رو فعالانه بالا ببره (با بازدهِ نزولی)، و رسیدن به
#  سطحِ exalted یه لقبِ همیشگی + پاداشِ کلان می‌ده. رابطه‌ی بالا با
#  پادشاه، رو بازارِ محلیِ همون نقشه هم تخفیف می‌ده (city_markets.py).
# ============================================================
...
def _king_kb(map_name: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🎁 تقدیمِ پیشکش", callback_data=f"king:tribute_menu:{map_name}", style=ButtonStyle.SUCCESS)],
...
def _tribute_kb(map_name: str) -> InlineKeyboardMarkup:
    ...
    rows.append([InlineKeyboardButton(text="🔙 برگشت", callback_data=f"king:open:{map_name}", style=ButtonStyle.PRIMARY)])
...
async def cb_king_tribute(cb: CallbackQuery):
    ...
    log_sync(
        f"👑 **KING TRIBUTE** — {player.get('name','—')} (`{uid}`) به {king['name']} ({map_name}) — "
...
def register_king_handlers(dp, bot):
    ...
    dp.callback_query.register(cb_king_tribute_menu, F.data.startswith("king:tribute_menu:"))
    dp.callback_query.register(cb_king_tribute, F.data.startswith("king:tribute:"))
