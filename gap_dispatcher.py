...
class GapDispatcher:
    ...
    async def _dispatch(self, payload: dict):
        ...
        # ─── 🆕 تشخیصِ گروه‌بودنِ چت (heuristic) ────────────────────
        # مستنداتِ رسمیِ گپ فیلدِ صریحِ chat_type نداره (یا لااقل پیدا
        # نشد)، ولی طبقِ همون نکته‌ای که بالاتر گفته شد («تو گروه‌ها
        # chat_id مالِ خودِ گروهه، نه فرستنده»)، یه سنجه‌ی قابل‌اتکا اینه:
        # اگه شناسه‌ی فرستنده (sender_id) با خودِ chat_id فرق داشته باشه،
        # یعنی این پیام تو یه فضای مشترک (گروه) اومده؛ تو چتِ خصوصی این
        # دوتا معمولاً برابرن. اگه بعداً معلوم شد اشتباهه (یا گپ رسماً
        # فیلدِ chat_type رو مستند کرد)، فقط همین خط رو عوض کن.
        is_group = bool(
            sender_id is not None and chat_id is not None and int(sender_id) != int(chat_id)
        )
        ...
        if msg_type == "text":
            ...
            msg = Message(self.bot, chat_id, None, text, user, raw=payload, chat_type=chat_type)
    ...
    async def _dispatch_message(self, msg: Message, text: str):
        ...
        # منشن ربات بزنن (مثلاً /characters@ASTRALABYSS_BOT) تا مطمئن
