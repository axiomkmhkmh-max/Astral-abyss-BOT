...
def class_card_text(player: dict) -> str:
    ...
    # 🏔 اگه بازیکن مسیرِ ارتقای کلاسی گرفته باشه، یه خط بهش اضافه می‌شه
    ascension_line = ""
    try:
        from class_ascension_system import current_ascension_path
        _ap = current_ascension_path(player)
        if _ap:
            ascension_line = f"🏔 مسیرِ ارتقا: {_ap['emoji']} {_ap['name_fa']}\n"
    except Exception:
        pass
