...
def _render_wizard_panel(player: dict) -> tuple[str, InlineKeyboardMarkup]:
    ...
    mastery = ca.wizard_mastery_bonus(player)
    text = (
        f"🧙‍♂️ **قدرت‌های جادوگر**\n{'─'*22}\n"
        f"🔹 مانا: {csd.get('mana',0)}/{csd.get('max_mana',0)}\n"
        f"🔹 عناصرِ بازشده: {known_fa}\n"
        f"🔹 شارژِ سپرِ مانا: {csd.get('mana_shield_charges',0)}\n"
        f"🔹 تسلطِ آرکین: {mastery['level']}/{ca.WIZARD_MASTERY_CAP} طلسم "
        f"(+{mastery['dmg_mult_add']*100:.1f}٪ دمیج، +{mastery['crit_add']*100:.1f}٪ کریت، +{mastery['lifesteal']*100:.1f}٪ لایف‌استیل)\n\n"
        f"🔥 **طلسمِ ترکیبی** ({ca.WIZARD_SPELL_COST} مانا) — ضربه‌ی بعدیِ نبردت تضمینی به ضعفِ دشمن می‌خوره + دمیجِ اضافه + یه پله تسلطِ آرکین\n"
...
def _render_merchant_panel(player: dict) -> tuple[str, InlineKeyboardMarkup]:
    ...
    fight = _get_fight(player)
    energy = csd.get("trade_energy", 0)
    max_energy = csd.get("max_trade_energy", 40)
    haggle_txt = "آماده ✅" if energy >= ca.HAGGLE_COST else f"⚡ {energy}/{max_energy}"
    text = (
        f"💰 **قدرت‌های تاجر**\n{'─'*22}\n"
        f"🔹 انرژیِ معامله: {energy}/{max_energy}\n"
        f"🔹 نفوذِ بازار (کل چانه‌زنی‌های موفق): {csd.get('market_influence',0)}\n"
        ...
        f"🤝 **چانه‌زنی** ({ca.HAGGLE_COST} انرژی) — شانسیِ ضریبِ درآمدِ طلا رو بالا می‌بره\n"
...
async def cb_merchant_haggle(cb: CallbackQuery):
    ...
    await asave_player(uid, player)
    if r["success"]:
        await cb.answer(f"🤝 چانه‌زنی موفق بود! ضریبِ طلا رفت رو ×{r['mult']:.3f}", show_alert=True)
    else:
