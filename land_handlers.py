...
def _home_kb(uid: int, player: dict) -> InlineKeyboardMarkup:
    doc = await bridge.land_system.get_my_land(uid)
    rows = []
    if doc:
...
async def cmd_land(msg: Message):
    ...
    if not player:
        ...
        return
    await msg.answer("🗺 **زمین**\n\n" + await bridge.land_system.land_summary_text(uid), reply_markup=_home_kb(uid, player))
...
async def cb_land_map(cb: CallbackQuery):
    ...
    if not _owner_ok(cb, uid):
        await cb.answer("❌", show_alert=True)
        return
    plots = await bridge.land_system.list_plots(map_name)
    await cb.answer()
...
async def cb_land_buy(cb: CallbackQuery):
    ...
    if not _owner_ok(cb, uid):
        ...
        return
    player = await aget_player(uid)
    ok, msg = await bridge.land_system.buy_land(uid, player, map_name, plot_id)
    if ok:
...
async def cb_land_expand(cb: CallbackQuery):
    ...
    if not _owner_ok(cb, uid):
        ...
        return
    player = await aget_player(uid)
    ok, msg = await bridge.land_system.expand_land(uid, player)
    if ok:
        await asave_player(uid, player)
...
async def cb_land_abandon(cb: CallbackQuery):
    ...
    if not _owner_ok(cb, uid):
        ...
        return
    player = await aget_player(uid)
    ok, msg = await bridge.land_system.abandon_land(uid, player)
    if ok:
        await asave_player(uid, player)
...
async def cb_land_collect(cb: CallbackQuery):
    ...
    if not _owner_ok(cb, uid):
        await cb.answer("❌", show_alert=True)
        return
    ok, msg = await bridge.land_system.collect_rent_income(uid)
    if ok:
...
async def cmd_landsell(msg: Message):
    ...
    if len(parts) != 2 or not parts[1].isdigit():
        ...
        return
    ok, m = await bridge.land_system.list_for_sale(uid, int(parts[1]))
    await msg.answer(m)
    if ok:
...
async def cb_land_unlist(cb: CallbackQuery):
    ...
    if not _owner_ok(cb, uid):
        await cb.answer("❌", show_alert=True)
        return
    ok, msg = await bridge.land_system.cancel_listing(uid)
    await cb.answer(msg, show_alert=True)
    player = await aget_player(uid)
    await cb.message.edit_text("🗺 **زمین**\n\n" + await bridge.land_system.land_summary_text(uid), reply_markup=_home_kb(uid, player))
...
async def cb_land_market(cb: CallbackQuery):
    ...
    if not _owner_ok(cb, uid):
        await cb.answer("❌", show_alert=True)
        return
    listings = await bridge.land_system.list_for_sale_all()
    await cb.answer()
...
async def cb_land_buylisted(cb: CallbackQuery):
    ...
    if not _owner_ok(cb, uid):
        ...
        return
    player = await aget_player(uid)
    ok, msg = await bridge.land_system.buy_listed_land(uid, player, map_name, plot_id)
    if ok:
        await asave_player(uid, player)
...
async def cmd_landrent(msg: Message):
    ...
    if len(parts) != 2 or not parts[1].isdigit():
        ...
        return
    ok, m = await bridge.land_system.set_rent_price(uid, int(parts[1]))
    await msg.answer(m)
    if ok:
...
async def cmd_rentland(msg: Message):
    ...
    ok, m = await bridge.land_system.rent_land(uid, player, map_part.strip(), plot_id)
    if ok:
        await asave_player(uid, player)
