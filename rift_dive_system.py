...
# 🩹 بالانس/باگ‌فیکس اضطراری — قبلاً هیچ سقفی برای برکت‌های معبد نبود.
# یه بازیکن با ده‌ها بار گرفتنِ همون برکت‌ها (مخصوصاً «پیوندِ خونی») به
# رانی رسید که عملاً کشته نمی‌شد (شفای هر برد از آسیبِ باختن جلو می‌زد) و
# هم‌زمان چون _status_text همه‌ی برکت‌ها رو تک‌تک تو یه پیام می‌نوشت،
# پیام از سقفِ طول تلگرام رد می‌شد و هر اکشنی (خروج/کش‌اوت) با خطای
# عمومی شکست می‌خورد. این ثابت‌ها هم جلوی رانِ جدید رو می‌گیرن، هم پایین‌تر
# تو _run() هر رانِ قدیمیِ رد از این سقف رو خودکار اصلاح می‌کنن.
MAX_BLESSINGS = 15
MAX_LIFESTEAL = 0.35
MAX_DEPTH = 90
...
def _sanitize_run(run: dict) -> None:
    """🩹 هر رانِ فعالی که از سقفِ فعلی رد شده رو (چه رانِ جدید بعداً بخواد
    رد بشه، چه رانِ قدیمی‌ای که قبل از این پچ رد شده بود) برمی‌گردونه رو
    سقف — بدونِ اینکه پاداشِ pending/banked‌ش دست بخوره. کافیه یه‌بار این
    تابع صدا زده بشه (روی هر لمسِ رانی که همینه) تا رانِ گیرکرده خودش
    درست بشه."""
    if len(run.get("blessings", [])) > MAX_BLESSINGS:
        run["blessings"] = run["blessings"][:MAX_BLESSINGS]
    if run.get("lifesteal", 0.0) > MAX_LIFESTEAL:
        run["lifesteal"] = MAX_LIFESTEAL
    if run.get("power_mult", 0.0) > 4.0:
        run["power_mult"] = 4.0
    if run.get("incoming_mult", 0.0) > 2.0:
        run["incoming_mult"] = 2.0
...
def _new_run() -> dict:
    return {
        ...
        "pending_shards": 0,
        "pending_items": [],
        "pending_materials": [],  # 🆕 موادِ خام (مثلِ تکه‌ی روح) — مثلِ pending_items با مرگ از دست می‌ره
...
def enter_next_room(player: dict) -> dict:
    ...
    🩹 بعد از MAX_DEPTH دیگه اتاقِ جدید نمی‌سازه — باید کش‌اوت کنی (جلوی
    رانِ بی‌نهایت‌طولانی و تجمعِ نامحدودِ برکت/آیتم رو می‌گیره)."""
    run = _run(player)
    if run["depth"] >= MAX_DEPTH:
        room = {"type": "max_depth", "resolved": True}
        run["room"] = room
        run["log"] = ["🏁 به عمقِ نهایی رسیدی — این شکاف دیگه عمیق‌تر نمی‌شه. باید کش‌اوت کنی."]
        return room
    run["depth"] += 1
...
def resolve_current_room(player: dict) -> dict:
    ...
    if rtype == "max_depth":
        room["resolved"] = True
        log = ["🏁 به عمقِ نهایی رسیدی — این شکاف دیگه عمیق‌تر نمی‌شه. باید کش‌اوت کنی."]
        run["log"] = log
        return {"ok": True, "log": log, "dead": False, "room_type": rtype}
    ...
    if rtype in ("combat", "elite"):
        ...
        if won:
            ...
            # 🆕 راهِ دومِ کسبِ «تکه‌ی روح» غیر از بازارِ سیاه — فقط از اتاقِ نخبه،
            # شانسِ کم. مثلِ بقیه‌ی pending، اگه قبل از دروازه بمیری از دست می‌ره.
            if is_elite and random.random() < 0.10:
                run["pending_materials"].append("soul_shard")
                log.append("🔹 یه تکه‌ی روح هم از نخبه افتاد!")
        else:
...
def choose_shrine(player: dict, option_id: str) -> list[str]:
    ...
    if len(run["blessings"]) >= MAX_BLESSINGS:
        room["resolved"] = True
        log = [f"✨ به سقفِ برکت‌ها رسیدی ({MAX_BLESSINGS}) — این معبد بدونِ اثر رد شد."]
        run["log"] = log
        return log
    ...
    run["lifesteal"] = min(MAX_LIFESTEAL, run["lifesteal"] + opt.get("lifesteal", 0.0))
...
def extract_at_gate(player: dict) -> dict:
    ...
    run["banked_materials"].extend(run["pending_materials"])
...
def finalize_run(player: dict, died: bool) -> dict:
    ...
    items = _generate_reward_items(player, run)  # ممکنه run["banked_zen"] رو (بابتِ آیتمِ مازاد بر سقف) بالا ببره
    ...
    if run["banked_materials"]:
        from katana_core import grant_soul_shard_item
        soul_shard_qty = sum(1 for m in run["banked_materials"] if m == "soul_shard")
        if soul_shard_qty:
            player.setdefault("inventory", []).extend(grant_soul_shard_item(soul_shard_qty))
    ...
    summary = {
        ...
        "soul_shards_gained": sum(1 for m in run["banked_materials"] if m == "soul_shard"),
        "lost_pending_zen": run["pending_zen"] if died else 0,
        "lost_pending_shards": run["pending_shards"] if died else 0,
        "lost_pending_materials": len(run["pending_materials"]) if died else 0,
    }
    player["rift_run"] = None
    return summary

MAX_ITEMS_PER_RUN = 40  # 🩹 جلوی تورمِ اینونتوری/سایزِ سیوِ بازیکن رو می‌گیره؛ مازاد به Zen تبدیل می‌شه

def _generate_reward_items(player: dict, run: dict) -> list[dict]:
    from item_system import generate_random_equipment
    banked = run["banked_items"]
    capped = banked[:MAX_ITEMS_PER_RUN]
    overflow = len(banked) - len(capped)
    items = []
    for entry in capped:
    ...
    if overflow > 0:
        # هر آیتمِ ردشده از سقف، به‌جاش ۵۰۰ Zen بانک می‌شه — پاداش از دست نمی‌ره
        run["banked_zen"] += overflow * 500
    return items
