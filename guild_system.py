...
# ─── Player Guild Data ───────────────────────────────────────
def ensure_guild_data(player: dict) -> bool:
    """
    🐛 فیکس: قبلاً همیشه True برمی‌گردوند (خودِ dict پلیر رو)، پس هر جا صدا
    زده می‌شد، صداکننده فکر می‌کرد «همیشه باید سیو کنم» و یه asave_player
    الکی روی *هر* بازدیدِ ساده‌ی صفحه‌ی گیلد (باز کردن/برگشتن) اجرا می‌شد.
    این سیوهای بی‌مورد و مکرر (بدونِ player_lock) دقیقاً پنجره‌ی race رو
    باز می‌کردن: اگه بینِ یه اکشنِ واقعیِ گیلد (که امتیازِ مشارکت اضافه
    می‌کنه) و سیوِ بعدیش، یه چنین سیوِ الکی‌ای از رویِ اسنپ‌شاتِ قدیمی‌تر
    اجرا می‌شد، امتیازِ تازه‌گرفته‌شده rewrite/overwrite و انگار «ریست»
    می‌شد. الان فقط وقتی واقعاً چیزی عوض بشه True برمی‌گردونه، تا صداکننده
    فقط اون‌موقع سیو کنه.
    """
    changed = False
