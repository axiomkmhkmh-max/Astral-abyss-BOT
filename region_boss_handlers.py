...
async def cb_region_boss_hit(cb: CallbackQuery):
    ...
    # 🔒 باگ‌فیکس: قبلاً ذخیره‌ی خودِ ضربه‌زن بدونِ player_lock بود — دقیقاً
    # همون کلاسِ باگِ دابل‌تپ/رِیسی که تو بقیه‌ی جاهای کدبیس (بازارِ سیاه،
    # بانک، ...) فیکس شده. چون باسِ منطقه ذاتاً چندنفره‌ست و خیلی وقتا
    # چندتا ضربه تقریباً هم‌زمان می‌رسن، این مسیر از همه بیشتر در خطرِ
    # همین باگ بود.
    ...
    if result["boss_killed"]:
        ...
        for ruid, r in rewards.items():
            # 🔒 باگ‌فیکس اصلی: اینجا دقیقاً همون جایی بود که آیتمِ لوت
            # (مثلِ «داسِ روحِ تاناتپ») گاهی گم می‌شد. قبلاً بدونِ قفل بود:
            # get_player → append آیتم به inventory → save_player. اگه تو
            # همین بازه‌ی کوتاه یه نوشتنِ دیگه‌ی هم‌زمان برای همون بازیکن
            # اتفاق می‌افتاد (خیلی محتمله چون خودِ گیرنده معمولاً همون
            # لحظه داره باز هم ضربه می‌زنه/کلیک می‌کنه)، اون نوشتنِ دیگه
            # می‌تونست کاملاً رویِ این یکی سوار بشه و آیتم رو بی‌سروصدا حذف
            # کنه — بدونِ هیچ خطایی، فقط لوت غیب می‌شد. حالا با player_lock
            # کلِ «بخون → اضافه کن → ذخیره کن» اتمیکه.
