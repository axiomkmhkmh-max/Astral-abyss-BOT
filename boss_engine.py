...
def distribute_rewards(boss: dict, is_weekly_featured: bool = False) -> dict:
    ...
    # 🆕 دراپِ کم‌شانسِ «تکه‌ی روح» از کشتنِ باس (جهانی یا منطقه‌ای) — راهِ
    # دومِ کسبش کنارِ بازارِ سیاه، تا بازیکن‌ها فقط وابسته‌ی سقفِ روزانه‌ی
    # شاپ نباشن. هر شرکت‌کننده‌ی واقعی (نه فقط تاپ‌سه) شانسِ مستقلِ کمی داره.
    from katana_core import grant_soul_shard_item
    SOUL_SHARD_DROP_CHANCE = 0.12
    for uid_str in contributors:
        uid = int(uid_str)
        if uid not in rewards:
            continue
        if random.random() < SOUL_SHARD_DROP_CHANCE:
            rewards[uid]["items"].extend(grant_soul_shard_item(1))
...
async def build_kill_summary(boss: dict, rewards: dict, speed_kill: bool, name_lookup) -> str:
    # 🐞 باگ‌فیکس: name_lookup یه تابعِ async ـه (چون باید await aget_player
    # بزنه)، ولی این تابع قبلاً sync بود و بدونِ await صداش می‌زد. نتیجه:
    # به‌جای اسمِ بازیکن، خودِ آبجکتِ coroutine (چیزی مثلِ
    # "<coroutine object _name_of at 0x...>") تو متنِ رده‌بندی چاپ می‌شد.
    # حالا خودِ تابع async شده و درست await می‌کنه.
