...
async def cb_core_unlock(cb: CallbackQuery):
    uid = cb.from_user.id
    # 🐛 باگ‌فیکس: قبلاً callback_data به‌شکلِ
    # f"core_unlock:{node_id}:{region}:{ring}" ساخته می‌شد، ولی خودِ
    # node_id از قبل به‌فرمِ "{region}:{ring}:{arm}" هست — یعنی خودش
    # دو تا ":" داره. پس رشته‌ی نهایی موقعِ split(":") به ۶ تیکه
    # می‌شکست، نه ۴ تا، و خطِ زیر با ValueError (too many values to
    # unpack) می‌ترکید — یعنی زدن رو هر ستاره‌ی نقشه ارور می‌داد.
    # چون node_id خودش region/ring رو داره، دیگه نیازی به تکرارشون
    # تو callback نیست؛ از روی CONSTELLATION می‌گیریمشون.
    _, node_id = cb.data.split(":", 1)
    node = cs.CONSTELLATION.get(node_id)
    if not node:
        await cb.answer("❌ این ستاره پیدا نشد!", show_alert=True)
        return
    region, ring = node["region"], node["ring"]
