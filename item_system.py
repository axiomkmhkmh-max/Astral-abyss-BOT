...
def format_item_card(item: dict) -> str:
    ...
    if item.get("katana_relic"):
        try:
            from katana_relic_abilities import get_relic_ability_info
            ab = get_relic_ability_info(item)
            if ab:
                kind_tag = "🎲 پسیو (همیشه فعال)" if ab["kind"] == "passive" else f"🎲 پروک ({int(ab.get('chance', 0) * 100)}٪ شانس هر ضربه)"
                lines.append(f"⚔️ **ابیلیتی:** {ab['emoji']} {ab['ability']} — {kind_tag}")
        except ImportError:
            pass
