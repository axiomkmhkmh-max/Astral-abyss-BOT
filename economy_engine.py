...
# جداگانه کمتر شد — طبق درخواستِ حسین، الان ثابت ۲۰٪ سرراسته. برخلافِ
# بقیه‌ی بازارها، این مالیات با هیچ لایه‌ی دیگه‌ای (افتِ ۷۰٪ حالتِ سخت،
# مالیاتِ نهنگ) جمع نمی‌شه — چون همون جمع‌شدنِ لایه‌ها بود که باعث می‌شد
# یه لوتِ ۱۲GZ فقط ۱GZ خالص بده، خیلی بیشتر از چیزی که پیام نشون می‌داد.
LOOT_SELL_TAX = 0.20
...
def apply_gold_find(player: dict, amount: int) -> int:
    ...
    try:
        ...
        pass
    try:
        # 🧵 بونوسِ دائمیِ رشته‌های سرنوشت (fate_threads.py)
        from fate_threads import get_fate_bonus
        bonus += get_fate_bonus(player, "gold_find_pct")
    except ImportError:
        pass
...
def compute_sell_tax(player: dict, amount: int, zone: str, market_id: str | None = None) -> dict:
    ...
    استثنا: لوت (market_id == "global_loot") از افتِ ۷۰٪ و مالیاتِ نهنگ معافه —
    فقط همون ۲۰٪ ثابتِ LOOT_SELL_TAX رو می‌گیره، طبق درخواستِ حسین."""
    is_loot = market_id == "global_loot"
    if not is_loot:
