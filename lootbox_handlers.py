# ============================================================
#  ASTRAL ABYSS — Lootbox Handlers (Telegram UI) 🎁
# ------------------------------------------------------------
#  زیرمنویِ «🎁 باکس شانسی» داخلِ بازارِ سیاه (bm:shop). منطق تو
#  lootbox_shop.py هست — این فایل فقط UI/دکمه‌هاست.
# ============================================================
...
def _lootbox_kb() -> InlineKeyboardMarkup:
    ...
    for box_id in lbx.BOX_ORDER:
        ...
        rows.append([InlineKeyboardButton(
            ...
            style=ButtonStyle.SUCCESS if box["currency"] == "zen" else ButtonStyle.PRIMARY,
        )])
    rows.append([InlineKeyboardButton(text="🔙 برگشت به فروشگاه", callback_data="bm:shop", style=ButtonStyle.PRIMARY)])
...
async def cb_lootbox_menu(cb: CallbackQuery):
    ...
    try:
        ...
        try:
            await cb.message.edit_caption(caption=_lootbox_text(player), reply_markup=_lootbox_kb())
        except Exception:
...
async def cb_lootbox_open(cb: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🎁 باکسِ دیگه", callback_data="bm:lootbox", style=ButtonStyle.SUCCESS)],
        [InlineKeyboardButton(text="🔙 برگشت به فروشگاه", callback_data="bm:shop", style=ButtonStyle.PRIMARY)],
...
def register_lootbox_handlers(dp: Dispatcher):
    dp.callback_query.register(cb_lootbox_menu, F.data == "bm:lootbox")
    dp.callback_query.register(cb_lootbox_open, F.data.startswith("lootbox_open:"))
