...
RESOURCE_KEYS = {
    ...
    "merchant":   ("trade_energy", "max_trade_energy", "trade_energy_regen"),
}
...
def tick_regen(player: dict) -> None:
    ...
    # 🩹 مهاجرت: تاجرهایی که قبل از اضافه‌شدنِ «انرژیِ معامله» ساخته شدن
    # فیلدهای جدید رو ندارن — بدونش max_key هم صفر می‌موند و ریجن برای
    # همیشه قفل می‌شد. این‌جا یه‌بار مقدارِ پیش‌فرض پر می‌شه.
    if cls == "merchant" and "max_trade_energy" not in csd:
        csd.setdefault("trade_energy", 40)
        csd.setdefault("max_trade_energy", 40)
        csd.setdefault("trade_energy_regen", 4)
...
def resource_line(player: dict) -> str:
    ...
    if cls in RESOURCE_KEYS:
        ...
        line = f"🔹 {c.get('resource_label_fa','منبع')}: {csd.get(res_key,0)}/{csd.get(max_key,0)}"
        if cls == "merchant":
            line += f" | ×{csd.get('gold_multiplier',1.0):.2f} طلا"
        return line
    return "🔹 منبع: —"
...
# ════════════════════════════════════════════════════════════
WIZARD_SPELL_COST = 18   # ⚖️ بافِ جادوگر: کمی ارزون‌تر شد تا با مانای ۵۰، ۲-۳ بار پشتِ‌سرِهم بشه زد و سریع‌تر به سقفِ Mastery رسید
WIZARD_SHIELD_COST = 15
WIZARD_NOVA_COST = 30
WIZARD_MASTERY_CAP = 100  # بعد از ۱۰۰ طلسمِ ترکیبیِ موفق، بونوسِ Mastery دیگه رشد نمی‌کنه
_WIZARD_ALL_ELEMENTS = ["fire", "water", "lightning"]
_WIZARD_ELEMENT_FA = {"fire": "🔥 آتش", "water": "❄️ آب/یخ", "lightning": "⚡ رعد"}


def wizard_mastery_bonus(player: dict) -> dict:
    """🆕 بافِ جادوگر — تسلطِ آرکین: موازیِ سیستمِ Bond ماجراجو (که با
    کشتنِ دشمن رشد می‌کنه)، این‌جا با تعدادِ طلسمِ ترکیبیِ موفق
    (synergy_combos_used — از قبل ثبت می‌شد ولی هیچ اثری نداشت) یه بونوسِ
    دائمی و تجمعی باز می‌شه. سقفش (۱۰۰ طلسم، هرکدوم مانا/زمان می‌بره)
    تقریباً هم‌ارزِ زحمتِ رسیدنِ ماجراجو به Bond سطح ۱۰.
    در سقف: +۲۰٪ دمیج، +۸٪ کریت، +۵٪ لایف‌استیل — نزدیک به پرک‌های نهاییِ
    Bond (+۸٪ دمیج، +۱۰٪ کریت، +۵٪ لایف‌استیل)."""
    if player.get("class") != "wizard":
        return {"dmg_mult_add": 0.0, "crit_add": 0.0, "lifesteal": 0.0, "level": 0}
    csd = player.get("class_system_data", {})
    used = min(csd.get("synergy_combos_used", 0), WIZARD_MASTERY_CAP)
    return {
        "dmg_mult_add": round(used * 0.002, 4),   # سقف ۰.۲۰ در ۱۰۰
        "crit_add":     round(used * 0.0008, 4),  # سقف ۰.۰۸ در ۱۰۰
        "lifesteal":    round(used * 0.0005, 4),  # سقف ۰.۰۵ در ۱۰۰
        "level": used,
    }
...
def wizard_arcane_nova(player: dict, enemy: dict | None) -> dict:
    ...
    dmg = int((atk * 2.6 + player.get("level", 1) * 5) * random.uniform(0.9, 1.3))  # ⚖️ بافِ جادوگر: ضریب و فلتِ لول بیشتر شد
...
HAGGLE_COST = 15  # ⚖️ بالانس: قبلاً کولداونِ ۱ ساعته با شانسِ رایگان بود؛
                   # الان مثلِ بقیه‌ی کلاس‌ها از استخرِ «انرژیِ معامله»
                   # مصرف می‌کنه — با پول ۴۰ و ریجنِ ۴، دقیقاً هر ۱۵ دقیقه
                   # (مثلِ مانا/استامینا/فیض) کامل پر می‌شه.
...
def merchant_haggle(player: dict) -> dict:
    """چانه‌زنی: مثلِ بقیه‌ی قابلیت‌های کلاسی، انرژیِ معامله خرج می‌کنه و
    شانسی ضریبِ درآمدِ طلا (gold_multiplier) رو کمی بالا می‌بره.
    ⚖️ بالانس: چون الان تا ۲-۳ برابر بیشتر (هر ۱۵ دقیقه به‌جای هر ۱ ساعت)
    قابلِ استفاده‌ست، مقدارِ هر بار کمتر شد تا سرعتِ رسیدن به سقفِ ×۲.۰۰
    تقریباً هم‌ارزِ نسخه‌ی قبلی بمونه، نه چند برابرِ سریع‌تر."""
    if player.get("class") != "merchant":
        return {"ok": False, "msg": "❌ این قابلیت مخصوصِ تاجره."}
    tick_regen(player)
    csd = player.setdefault("class_system_data", {})
    if csd.get("trade_energy", 0) < HAGGLE_COST:
        return {"ok": False, "msg": f"❌ انرژیِ معامله کافی نیست! ({csd.get('trade_energy',0)}/{HAGGLE_COST})"}
    csd["trade_energy"] -= HAGGLE_COST
    success = random.random() < 0.7
    if success:
        gain = round(random.uniform(0.002, 0.005), 4)
        csd["gold_multiplier"] = round(min(2.0, csd.get("gold_multiplier", 1.0) + gain), 4)
        csd["market_influence"] = csd.get("market_influence", 0) + 1
        return {"ok": True, "success": True, "gain": gain, "mult": csd["gold_multiplier"], "energy_left": csd["trade_energy"]}
    return {"ok": True, "success": False, "energy_left": csd["trade_energy"]}
