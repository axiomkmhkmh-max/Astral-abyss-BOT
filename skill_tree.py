...
def get_skill_bonuses(player: dict) -> dict:
    ...
    # 🌌 Living Class Core (constellation_system.py) — لایه‌ی دومِ باف که
    # از صورت‌فلکِ بدونِ مرزِ کلاس‌ها میاد. جمعش می‌زنیم تو همین dict تخت
    # تا combat/pvp/economy/loot که همه از get_skill_bonuses می‌خونن،
    # بدونِ هیچ تغییرِ دیگه‌ای این باف رو هم خودکار ببینن.
    try:
        from constellation_system import get_core_bonuses
        for key, val in get_core_bonuses(player).items():
            bonuses[key] = bonuses.get(key, 0) + val
    except ImportError:
        pass
    return bonuses
