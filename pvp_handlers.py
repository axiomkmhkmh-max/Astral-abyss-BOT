...
def _lobby_text_and_kb(uid: int) -> tuple[str, InlineKeyboardMarkup]:
    ...
    for pid, p in others[:25]:
        ...
        if online and not busy:
            buttons.append([InlineKeyboardButton(text=f"⚔️ چالش {p['name']}", callback_data=f"pvp_challenge:{pid}", style=ButtonStyle.DANGER)])

    if shown == 0:
        lines.append("\n😔 هیچ بازیکنی پیدا نشد!")

    lines.append(f"\n📝 `/duel @username` یا `/duel random` (نزدیک‌ترین حریف از نظرِ قدرت) هم کار می‌کنه.")
    buttons.append([InlineKeyboardButton(text="🛡️ PvP تیمی (۲به۲ تا ۵به۵)", callback_data="pvp_goto_team", style=ButtonStyle.DANGER)])
    buttons.append([InlineKeyboardButton(text="🔙 بستن", callback_data="pvp_close", style=ButtonStyle.PRIMARY)])
...
async def cb_pvp_menu_back(cb: CallbackQuery, bot: Bot):
...
async def cb_pvp_goto_team(cb: CallbackQuery):
    from team_pvp_handlers import _menu_text_and_kb
...
async def _find_closest_cp_opponent(uid: int, players: dict) -> int | None:
    ...
    پیدا می‌کنه و از بینشون یکی رو انتخاب می‌کنه — تا مسابقه‌ی نابرابر کمتر بشه
    ولی همچنان کمی غافلگیری/تنوع بمونه."""
    import random as _r
    ...
    for pid, p in players.items():
        pid_int = int(pid)
        if pid_int == uid or not p.get("class") or get_fight_by_uid(pid_int):
            continue
...
async def cb_pvp_challenge(cb: CallbackQuery, bot: Bot):
    uid = cb.from_user.id
...
async def _send_stake_prompt(msg: Message, uid: int, target_id: int, is_challenger: bool, edit: bool = False):
    ...
    buttons = [
        [InlineKeyboardButton(text=f"💰 {amt:,}", callback_data=f"pvp_stakeset:{target_id}:{amt}", style=ButtonStyle.PRIMARY)]
        for amt in STAKE_OPTIONS
    ]
    buttons.append([InlineKeyboardButton(text="🚫 بدون شرط", callback_data=f"pvp_stakeset:{target_id}:0", style=ButtonStyle.PRIMARY)])
    buttons.append([InlineKeyboardButton(text="🔙 برگشت", callback_data="pvp_menu_back", style=ButtonStyle.PRIMARY)])
...
async def cb_pvp_stakeset(cb: CallbackQuery, bot: Bot):
...
async def _cb_pvp_stakeset_body(cb: CallbackQuery, bot: Bot):
    uid = cb.from_user.id
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="✅ قبول مبارزه", callback_data=f"pvp_accept:{uid}", style=ButtonStyle.SUCCESS),
        InlineKeyboardButton(text="❌ رد کن", callback_data=f"pvp_reject:{uid}", style=ButtonStyle.DANGER),
    ]])
...
async def _expire_duel(challenger_uid: int, bot: Bot):
    ...
    for tid in (challenger_uid, duel["target_id"]):
        try:
            await bot.send_message(tid, "⏰ **درخواستِ دوئل منقضی شد!** جواب داده نشد.")
        except Exception:
            pass


async def cb_pvp_reject(cb: CallbackQuery, bot: Bot):
    ...
    try:
        ...
        pass
    try:
        await bot.send_message(challenger_uid, "❌ **درخواستِ دوئلت رد شد!** حریفت نمی‌خواد بجنگه.")
    except Exception:
        pass
    await cb.answer()


async def cb_pvp_accept(cb: CallbackQuery, bot: Bot):
    uid = cb.from_user.id
    challenger_uid = int(cb.data.split(":")[1])
    duel = pending_duels.get(challenger_uid)
...
# شروعِ فایت
# ────────────────────────────────────────────────────────────
async def _start_fight(bot: Bot, chat_id: int, uid1: int, uid2: int, p1_data: dict, p2_data: dict, stake: int):
    char1 = ALL_CHARACTERS.get(p1_data.get("character"), {})
    char2 = ALL_CHARACTERS.get(p2_data.get("character"), {})

    # ─── کدکس: هرکدوم، کاراکترِ اون یکی رو تو کدکسش کشف می‌کنه ────
    try:
        from codex_system import mark_seen, check_and_claim_milestones
        codex_msgs = []
        mark_seen(p1_data, p2_data.get("character", ""))
        mark_seen(p2_data, p1_data.get("character", ""))
        codex_msgs += check_and_claim_milestones(p1_data)
        codex_msgs += check_and_claim_milestones(p2_data)
        await asave_player(uid1, p1_data)
        await asave_player(uid2, p2_data)
    except Exception:
        pass
...
def _action_kb(fight: FightSession, fighter: FighterState) -> InlineKeyboardMarkup:
    ...
    for i, ab in enumerate(fighter.abilities):
        ...
        ab_row.append(InlineKeyboardButton(text=label, callback_data=f"pvpfight_ab:{fight.fight_id}:{fighter.uid}:{i}", style=ButtonStyle.PRIMARY))
        if len(ab_row) == 2:
            rows.append(ab_row)
            ab_row = []
    if ab_row:
        rows.append(ab_row)
    rows.append([
        InlineKeyboardButton(text="⚔️ حمله معمولی", callback_data=f"pvpfight_atk:{fight.fight_id}:{fighter.uid}", style=ButtonStyle.DANGER),
        InlineKeyboardButton(text="🛡 دفاع", callback_data=f"pvpfight_def:{fight.fight_id}:{fighter.uid}", style=ButtonStyle.PRIMARY),
    ])
    return InlineKeyboardMarkup(inline_keyboard=rows)


async def _send_turn_prompt(fight: FightSession, fighter: FighterState, bot: Bot):
    ...
    try:
        m = await bot.send_message(fighter.uid, text, reply_markup=kb)
        fight.prompt_msgs[fighter.uid] = m.message_id
    except Exception as e:
        # باگ اصلی همینجا بود: اگه پیامِ خصوصی نمی‌رسید (مثلاً بازیکن هنوز
        # به ربات /start نزده بود)، این خطا بی‌صدا قورت داده می‌شد و هیچ
        # دکمه‌ای هیچ‌جا نشون داده نمی‌شد. بازیکن هیچ‌وقت شانسِ انتخاب
        # نداشت، نوبتش با تایم‌اوت خودکار «حمله‌ی معمولی» پر می‌شد و از
        # دیدِ اون دقیقاً انگار ربات به‌جاش تصمیم گرفته. حالا:
        # ۱) خطا لاگ می‌شه تا قابلِ دیدن باشه، ۲) دکمه‌ها به‌عنوانِ فال‌بک
        # تو همون گروهی که فایت توش شروع شده فرستاده می‌شن تا بازیکن هنوز
        # بتونه خودش نوبتش رو انتخاب کنه.
        log_sync(
            f"⚠️ **PVP DM FAILED** — نتونستم به `{fighter.uid}` ({fighter.name}) پیامِ خصوصیِ نوبت رو بدم: `{e}`. فال‌بک به گروه.",
            "PVP"
        )
        try:
            mention = f"[{fighter.name}](tg://user?id={fighter.uid})"
            m = await bot.send_message(
                fight.chat_id,
                f"⚠️ {mention} چون نتونستم بهت پیامِ خصوصی بدم (اول باید ربات رو /start بزنی)، "
                f"فعلاً نوبتت رو همینجا انتخاب کن:\n\n{text}",
                reply_markup=kb,
            )
            fight.prompt_msgs[fighter.uid] = m.message_id
        except Exception:
...
async def _register_action(fight: FightSession, uid: int, action: dict, bot: Bot):
...
async def _resolve_and_advance(fight: FightSession, bot: Bot):
...
async def _turn_timeout_watcher(fight_id: str, turn_at_spawn: int, bot: Bot):
...
def _check_actor(cb: CallbackQuery, parts: list[str]) -> bool:
    """اگه دکمه (چه تو پی‌وی، چه فال‌بکِ گروه) برای این کاربر نبود، رد می‌کنه.
    این جلوی این حالت رو می‌گیره که تو فال‌بکِ گروهی، حریف یا هرکسِ دیگه
    به‌جای بازیکنِ واقعی دکمه‌ی نوبتِ اونو بزنه."""
    if len(parts) > 2:
        try:
    ...
    return True  # فرمتِ قدیمی (بدون uid) — برای سازگاری، رد نمی‌شه


async def cb_fight_attack(cb: CallbackQuery, bot: Bot):
    parts = cb.data.split(":")
    fight_id = parts[1]
    fight = active_fights.get(fight_id)
    if not fight:
...
async def cb_fight_defend(cb: CallbackQuery, bot: Bot):
    parts = cb.data.split(":")
    fight_id = parts[1]
    fight = active_fights.get(fight_id)
    if not fight:
...
async def cb_fight_ability(cb: CallbackQuery, bot: Bot):
    parts = cb.data.split(":")
    fight_id = parts[1]
    fight = active_fights.get(fight_id)
    if not fight:
...
# ────────────────────────────────────────────────────────────
async def _end_fight(fight: FightSession, bot: Bot):
    """محکم‌سازی: هرچی تو محاسبه‌ی جایزه/رنک (بونتی، daily_wanted،
    achievements و ...) بترکه، فایت بازم قطعاً از player_in_fight/
    active_fights پاک می‌شه. باگِ اصلیِ «هربار میرم PvP فقط نوبتِ ۱ رو
    می‌بینم» همینجا بود: اگه وسطِ این تابع یه Exception می‌افتاد، phase
    رو 'ended' می‌ذاشتیم ولی pop نهایی هیچ‌وقت اجرا نمی‌شد — پس هر دو
    بازیکن برای همیشه «توی یه فایتی» قفل می‌موندن (get_fight_by_uid
    همیشه truthy برمی‌گردوند) و هیچ /duel یا چالشِ جدیدی جواب نمی‌داد،
    فقط یه popup بی‌صدا («⚔️ الان توی یه فایتی!») که تو چتِ خودِ کاربر
    هیچ پیامِ جدیدی نمی‌ذاشت — دقیقاً همون حسِ «فقط همینو نشونم می‌ده»."""
    p1, p2 = fight.p1, fight.p2
    try:
        await _end_fight_body(fight, bot)
    except Exception as e:
        log_sync(
            f"🔴 **PVP END-FIGHT CRASH** — محاسبه‌ی نتیجه‌ی فایت `{fight.fight_id}` "
            f"با خطا شکست خورد: `{e}`. فایت به‌صورتِ اضطراری بسته شد تا بازیکن‌ها قفل نمونن.",
            "ERROR",
        )
        try:
            await bot.send_message(
...
async def _end_fight_body(fight: FightSession, bot: Bot):
    p1, p2 = fight.p1, fight.p2
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="🔄 انتقام", callback_data="pvp_revenge_btn", style=ButtonStyle.PRIMARY),
        InlineKeyboardButton(text="📊 آمار کامل", callback_data="pvp_stats_btn", style=ButtonStyle.PRIMARY),
    ]])
    try:
    ...
    if new_league != old_league:
        try:
            ...
            await bot.send_photo(fight.chat_id, FSInputFile(out_path))
        except Exception as e:
...
# تسلیم‌شدن
# ────────────────────────────────────────────────────────────
async def cmd_forfeit(msg: Message, bot: Bot):
    uid = msg.from_user.id
...
# Register
# ────────────────────────────────────────────────────────────
def register_pvp_handlers(dp: Dispatcher, bot: Bot):
    dp.message.register(cmd_arena, F.text == "PvP")
    dp.message.register(cmd_arena, Command("arena"))
    dp.message.register(cmd_arena, Command("pvp"))
    ...
    dp.message.register(cmd_pvpseason, Command("pvpseason"))
    dp.message.register(cmd_revenge, Command("revenge"))
    dp.message.register(lambda m: cmd_forfeit(m, bot), Command("forfeit"))
    ...
    dp.callback_query.register(_w_pvp_challenge, F.data.startswith("pvp_challenge:"))
    dp.callback_query.register(_w_pvp_stakeset, F.data.startswith("pvp_stakeset:"))
    dp.callback_query.register(_w_pvp_accept, F.data.startswith("pvp_accept:"))
    dp.callback_query.register(_w_pvp_reject, F.data.startswith("pvp_reject:"))
    dp.callback_query.register(_w_pvp_menu_back, F.data == "pvp_menu_back")
    dp.callback_query.register(cb_pvp_close, F.data == "pvp_close")
    dp.callback_query.register(cb_pvp_goto_team, F.data == "pvp_goto_team")
    dp.callback_query.register(cb_pvp_revenge_btn, F.data == "pvp_revenge_btn")
    dp.callback_query.register(cb_pvp_stats_btn, F.data == "pvp_stats_btn")

    dp.callback_query.register(_w_fight_attack, F.data.startswith("pvpfight_atk:"))
    dp.callback_query.register(_w_fight_defend, F.data.startswith("pvpfight_def:"))
    dp.callback_query.register(_w_fight_ability, F.data.startswith("pvpfight_ab:"))
