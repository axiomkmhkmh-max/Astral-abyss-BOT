# ============================================================
#  ASTRAL ABYSS — 🎡 هندلرهای چرخِ شانسِ روزانه (Gap dispatcher)
# ============================================================
...
def _entry_kb(can_spin: bool) -> InlineKeyboardMarkup:
    rows = []
    if can_spin:
        rows.append([InlineKeyboardButton(text="🎡 بچرخون!", callback_data="dwheel:spin")])
...
async def cb_daily_wheel_spin(cb: CallbackQuery):
    ...
    log_sync(
        f"🎡 **DAILY WHEEL (Gap)** — {player.get('name','—')} (`{uid}`) — {seg['label']} "
...
def register_gap_daily_wheel_handlers(dp: GapDispatcher):
    dp.register_message(cmd_daily_wheel, text="🎡 چرخِ شانس")
    dp.register_message(cmd_daily_wheel, commands=["wheel"])
    dp.register_callback(cb_daily_wheel_spin, data="dwheel:spin")
