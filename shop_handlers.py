...
def _my_shop_kb(uid: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🏘 مغازه‌های دیگران", callback_data=f"shoplist:0:{uid}", style=ButtonStyle.PRIMARY)],
    ])


# ─── مرورِ مغازه‌های دیگران ────────────────────────────────────
SHOPS_PAGE_SIZE = 8


def _shops_list_kb(page: int, shops: list, total: int, uid: int) -> InlineKeyboardMarkup:
    buttons = []
    for s in shops:
        owner_id = s["_id"]
        shop_info = s.get("shop", {})
        shop_name = shop_info.get("name") or f"مغازه‌ی {s.get('name','ناشناس')}"
        n_items = len(shop_info.get("listings", []))
        label = f"🏪 {shop_name} ({n_items} کالا)"
        if owner_id == uid:
            label += " ⭐ (خودت)"
        buttons.append([InlineKeyboardButton(
            text=label, callback_data=f"shopview:{owner_id}:{page}:{uid}", style=ButtonStyle.PRIMARY)])
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="◀️ قبلی", callback_data=f"shoplist:{page-1}:{uid}", style=ButtonStyle.PRIMARY))
    if (page + 1) * SHOPS_PAGE_SIZE < total:
        nav.append(InlineKeyboardButton(text="بعدی ▶️", callback_data=f"shoplist:{page+1}:{uid}", style=ButtonStyle.PRIMARY))
    if nav:
        buttons.append(nav)
    buttons.append([InlineKeyboardButton(text="🏪 مغازه‌ی خودم", callback_data=f"shop_home:{uid}", style=ButtonStyle.PRIMARY)])
    return InlineKeyboardMarkup(inline_keyboard=buttons)


async def _shops_list_text_kb(page: int, uid: int):
    skip = page * SHOPS_PAGE_SIZE
    shops, total = await asyncio.gather(
        asyncio.to_thread(ss.list_active_shops, skip, SHOPS_PAGE_SIZE),
        asyncio.to_thread(ss.count_active_shops),
    )
    if not shops:
        return ("📭 فعلاً هیچ مغازه‌ی فعالی نیست. اولین نفر باش که یه کالا "
                "می‌ذاره تو مغازه‌ش! (`/shop`)"), None
    text = (f"🏘 **مغازه‌های فعالِ بازیکن‌ها** ({total} مغازه — صفحه {page+1})\n\n"
            f"روی هرکدوم بزن تا سر بزنی و خرید کنی:")
    return text, _shops_list_kb(page, shops, total, uid)


async def cmd_shops(msg: Message):
    uid = msg.from_user.id
    player = await aget_player(uid)
    if not player:
        await msg.answer("❌ اول باید بازی رو شروع کنی: /start")
        return
    text, kb = await _shops_list_text_kb(0, uid)
    await msg.answer(text, reply_markup=kb)


async def cb_shops_list(cb: CallbackQuery):
    _, page_s, uid_s = cb.data.split(":")
    page, uid = int(page_s), int(uid_s)
    if not _owner_ok(cb, uid):
        await cb.answer("❌", show_alert=True)
        return
    text, kb = await _shops_list_text_kb(page, uid)
    await cb.answer()
    await cb.message.edit_text(text, reply_markup=kb)


async def cb_shop_view_from_list(cb: CallbackQuery):
    _, owner_id_s, page_s, uid_s = cb.data.split(":")
    owner_id, page, uid = int(owner_id_s), int(page_s), int(uid_s)
    if not _owner_ok(cb, uid):
        await cb.answer("❌", show_alert=True)
        return
    if owner_id == uid:
        await cb.answer("😅 این مغازه‌ی خودته! از /shop مدیریتش کن.", show_alert=True)
        return
    owner = await aget_player(owner_id)
    if not owner:
        await cb.answer("❌ این بازیکن دیگه پیدا نشد.", show_alert=True)
        return
    text, kb = _visit_text_kb(owner, uid)
    kb.inline_keyboard.append([InlineKeyboardButton(
        text="◀️ بازگشت به لیست", callback_data=f"shoplist:{page}:{uid}", style=ButtonStyle.PRIMARY)])
    await cb.answer()
    await cb.message.edit_text(text, reply_markup=kb)
...
def register_shop_handlers(dp, bot):
    ...
    dp.message.register(cmd_shops, Command("shops"))
    dp.message.register(cmd_visit, Command("visit"))
    dp.callback_query.register(cb_shops_list,          F.data.startswith("shoplist:"))
    dp.callback_query.register(cb_shop_view_from_list, F.data.startswith("shopview:"))
