...
def location_kb(map_idx: int, map_name: str, player: dict | None = None) -> InlineKeyboardMarkup:
    ...
    buttons.append([InlineKeyboardButton(text="🗣️ دیدار با پادشاهِ شهر", callback_data=f"king:open:{map_name}", style=ButtonStyle.PRIMARY)])
    buttons.append([InlineKeyboardButton(text="🏮 بازارِ محلی", callback_data=f"cmkt:hub:{map_name}", style=ButtonStyle.SUCCESS)])
...
def bm_main_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🎁 باکس شانسی",       callback_data="bm:lootbox", style=ButtonStyle.SUCCESS)],
...
def register_loot_handlers(dp: Dispatcher, bot: Bot):
    ...
    from lootbox_handlers import register_lootbox_handlers
