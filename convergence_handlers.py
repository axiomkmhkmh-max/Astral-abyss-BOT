...
async def cmd_convergence(msg: Message):
    ...
    if not player:
        ...
        return
    state = await bridge.convergence_system.get_state()
    await msg.answer(await bridge.convergence_system.status_text(player), reply_markup=_kb(state.get("active", False)))
...
async def cb_conv_refresh(cb: CallbackQuery):
    ...
    state = await bridge.convergence_system.get_state()
    await cb.answer()
    await cb.message.edit_text(await bridge.convergence_system.status_text(player), reply_markup=_kb(state.get("active", False)))


async def cb_conv_top(cb: CallbackQuery):
    from database import get_player as gp
    top = await bridge.convergence_system.get_top_contributors(10)
    medals = ["🥇", "🥈", "🥉"] + ["🏅"] * 7
...
async def cb_conv_give_zen(cb: CallbackQuery, state: FSMContext):
    if not await bridge.convergence_system.is_active():
...
async def _do_contribute(msg: Message, state: FSMContext, kind: str):
    ...
    amount = int(txt)
    player = await aget_player(uid)
    result = await bridge.convergence_system.contribute(uid, player, kind, amount)
    if not result["ok"]:
    ...
    text = (
        ...
        f"{cv.progress_bar(await bridge.convergence_system.get_state())} {result['pct']}٪"
    )
    await msg.answer(text)

    if result["milestones_crossed"]:
        bot = msg.bot
        await cv._broadcast_milestone(bot, await bridge.convergence_system.get_state(), result["milestones_crossed"][-1])
