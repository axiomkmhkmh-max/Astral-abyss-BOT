...
# 🆕 راهِ دومِ کسبِ «تکه‌ی روح» — تا الان تنها منبعش بازارِ سیاه بود (سقفِ
# روزانه‌ی ۳ تا، یعنی ۵ روز تمام برای یه بیداریِ کامل روی تایرِ MYTHIC).
# این هلپر همون شکلِ دیکشنریِ آیتمِ بازارِ سیاه رو می‌سازه (name/emoji/type/
# sell/shop_exclusive) تا هرجایی که به‌عنوانِ دراپ/جایزه اضافه بشه، هم تو
# کوله‌پشتی درست نمایش داده بشه و هم با «فروشِ همه» قاطی نشه — دقیقاً مثلِ
# نسخه‌ی خریداری‌شده از دلالِ بیداری.
def grant_soul_shard_item(qty: int = 1) -> list[dict]:
    info = MATERIALS_INFO["soul_shard"]
    return [{
        "name": "soul_shard", "emoji": info["emoji"], "type": "awaken_material",
        "sell": 2400,  # ≈ ۳۰٪ قیمتِ بازارِ سیاه (۸۰۰۰)، برای هم‌خوانی با نسخه‌ی خریداری‌شده
        "shop_exclusive": True,
    } for _ in range(qty)]
