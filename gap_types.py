...
class GapUser:
    ...
    is_bot: bool = False
    # 🆕 آیا این پیام از یه گروهِ گپ اومده؟ (heuristic — نگاه کن به
    # توضیحِ کاملش تو gap_dispatcher._dispatch، همون‌جا محاسبه می‌شه)
    is_group: bool = False
...
class Message:
    ...
    def __init__(self, bot: "GapBotAdapter", chat_id: int, message_id: Optional[int],
        ...
        self.bot = bot
        # اگه صریحاً chat_type داده نشده، از رو is_group خودِ فرستنده حدس بزن
        # (پیش‌فرض معقول برای جاهایی که Message دستی ساخته می‌شه، مثلِ
        # answer() که یه Message جدید برمی‌گردونه).
        if chat_type is None:
            chat_type = "group" if getattr(from_user, "is_group", False) else "private"
...
class _Chat:
    id: int
    # "private" یا "group" — معادلِ aiogram's chat.type، برای گیت‌کردنِ
    # کلماتی که فقط باید تو گروه کار کنن (نگاه کن به gap_group_gate.py)
    type: str = "private"
