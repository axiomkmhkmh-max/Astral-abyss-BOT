...
def build_region_ring_kb(player: dict, region: str, ring: int) -> InlineKeyboardMarkup:
    ...
    if ring == 0:
        ...
        status = node_status(player, nid)
        if status == "available":
            rows.append([InlineKeyboardButton(text="🔓 باز کردن هاب", callback_data=f"core_unlock:{nid}:{region}:{ring}", style=ButtonStyle.PRIMARY)])
    else:
        row = []
        for arm in range(ARMS):
            nid = _node_id(region, ring, arm)
            ...
            label = f"{icon} ⭐{arm+1}"
            row.append(InlineKeyboardButton(text=label, callback_data=f"core_unlock:{nid}:{region}:{ring}", style=ButtonStyle.PRIMARY))
            if len(row) == 4:
                rows.append(row)
                row = []
        if row:
