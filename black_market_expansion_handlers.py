...
async def cb_bm_dealers(cb: CallbackQuery):
    ...
    try:
        await cb.message.edit_text(text, reply_markup=await asyncio.to_thread(_dealers_list_kb, uid))
    except Exception:
        await cb.message.answer(text, reply_markup=await asyncio.to_thread(_dealers_list_kb, uid))
    await cb.answer()
...
async def cb_deal_view(cb: CallbackQuery):
    ...
    if not player:
        await cb.answer("❌", show_alert=True); return
    doc = await bridge.black_market_dealers.get_dealer(map_name, player=player, uid=uid)
    if not doc:
...
async def cb_deal_buy(cb: CallbackQuery):
    ...
    # 🔒 باگ‌فیکس: قبلاً بدونِ player_lock بود — دقیقاً همون باگِ خریدِ تخمِ
    # پت. اگه رو دکمه‌ی خرید دوبار پشتِ‌سرِهم می‌زدی، دو تا کالبکِ هم‌زمان
    # هر دو با یه نسخه‌ی قدیمیِ player شروع می‌شدن (یکی‌شون Zen رو کم
    # می‌کرد و آیتم رو به انبار اضافه می‌کرد، بعد نوشتنِ دومی اون تغییرات
    # رو overwrite می‌کرد) — یعنی یا Zen بدونِ گرفتنِ آیتم کم می‌شد، یا
    # برعکس، بدونِ کم‌شدنِ Zen آیتم گم می‌شد.
    async with player_lock(uid):
        player = await aget_player(uid)
        if not player:
            await cb.answer("❌", show_alert=True); return

        ok, msg, info = await bridge.black_market_dealers.buy_from_dealer(player, map_name, row_id, uid=uid)
        if not ok:
