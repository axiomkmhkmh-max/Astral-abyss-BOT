...
# ============================================================
#  🎪 رویدادهای بازار (Market Events) — هر نقشه هر چرخه‌ی رفرش یه
#  حال‌وهوایِ متفاوت می‌گیره: گاهی ارزون، گاهی ملتهب، گاهی یه کاروانِ
#  مرموز از راه می‌رسه. تعیینِ رویداد seed‌دار و مشترکِ بینِ همه‌ی
#  بازیکن‌هاست (هرکی اول سر بزنه، رویدادِ اون چرخه رو "می‌سازه").
# ============================================================
MARKET_EVENT_TYPES: dict[str, dict] = {
    "normal": {
        "weight": 50, "label": "😐 روزِ عادی",
        "price_mult": 1.0, "special_chance_mult": 1.0,
        "banner": None,
    },
    "discount_carnival": {
        "weight": 16, "label": "🎉 جشنواره‌ی تخفیف",
        "price_mult": 0.80, "special_chance_mult": 1.0,
        "banner": "🎉 **جشنواره‌ی تخفیف تو راهه!** همه‌ی غرفه‌های این شهر امروز ۲۰٪ ارزون‌تر می‌فروشن.",
    },
    "price_surge": {
        "weight": 14, "label": "📈 التهابِ بازار",
        "price_mult": 1.25, "special_chance_mult": 1.8,
        "banner": "📈 **التهابِ بازار!** تقاضا زده بالا، قیمت‌ها گرون‌تر شدن — ولی شانسِ کالای ✨ویژه هم بیشتره.",
    },
    "black_caravan": {
        "weight": 12, "label": "🐫 کاروانِ سیاه",
        "price_mult": 1.0, "special_chance_mult": 1.0,
        "banner": "🐫 **کاروانِ سیاه از راه رسیده!** یه تاجرِ مرموز، گوشه‌ی بازار وایساده با یه کالای نایاب — تا رفرشِ بعدی همون‌جاست.",
    },
    "smugglers_night": {
        "weight": 8, "label": "🕶️ شبِ قاچاقچی‌ها",
        "price_mult": 0.90, "special_chance_mult": 2.6,
        "banner": "🕶️ **شبِ قاچاقچی‌هاست.** جنسِ دست‌اول با تخفیف، شانسِ کالای ✨ویژه هم رفته بالا — ولی فردا شاید دیگه خبری نباشه.",
    },
}
CARAVAN_EVENTS = ("black_caravan", "smugglers_night")
CARAVAN_PRICE_MULT = 2.6      # قیمتِ کالای کاروان نسبت به sell پایه‌ش
CARAVAN_RARITY_BUMP = 1        # چندتا پله بالاترِ سقفِ رریتیِ عادیِ همون نقشه

# ─── 🌌 کاروانِ نایاب — شانسِ کمی که خودِ کاروان یه پله دیگه هم بره بالاتر ─
ELITE_CARAVAN_CHANCE = 0.18
ELITE_CARAVAN_RARITY_BUMP = 2
ELITE_CARAVAN_PRICE_MULT = 4.2

# ─── 🔥 دماسنجِ بازار — بر اساسِ تعدادِ خریدِ کلِ بازیکن‌ها از یه غرفه، این‌چرخه ─
HYPE_TIERS = [
    (12, "🔥🔥🔥 بازار ترکونده"),
    (6,  "🔥🔥 خیلی داغ"),
    (3,  "🔥 داغ"),
...
# ─── 🎁 جکپاتِ چانه‌زنی — شانسِ کوچیکِ یه هدیه‌ی رایگان روی چانه‌زنیِ موفق ─
HAGGLE_JACKPOT_CHANCE = 0.10

# ─── ✨ واکنش‌های بازار به خریدهای خفن (رریتیِ بالا) ──────────────────
EPIC_REACTION_MIN_INDEX = 4  # از mythic به بالا (رجوع به RARITY_ORDER)
EPIC_PURCHASE_REACTIONS = [
    "✨ همهمه‌ی بازار یهو ساکت شد... انگار همه فهمیدن چی دستته.",
    "🌟 برای یه لحظه، نورِ کمرنگی از جنسِ خودِ کالا دستتو روشن کرد.",
    "⚡ فروشنده با تعجب زیرِ لب زمزمه کرد: «خیلی وقت بود کسی جرأتِ خریدنِ این‌و نداشت.»",
    "🔥 چند نفر از دورتر برگشتن نگاهت کردن — این‌جور چیزها زود سروصدا می‌کنه.",
    "🌌 برای یه ثانیه، انگار خودِ بازار نفسش رو حبس کرد.",
]

# ─── 🗣️ شایعه‌های محلی — یه تیکه‌کلامِ اتمسفریک هر بار که وارد بازار می‌شی ─
HUB_RUMORS = [
    "شایعه شده یکی دیروز تو یکی از غرفه‌ها یه چیزِ عجیب پیدا کرده بود...",
    "می‌گن قیمت‌ها هر چرخه دستِ خودِ بازاره، نه فروشنده‌ها.",
    "یه پیرمرد کنارِ ورودی زمزمه می‌کرد: «امروز بازار بو-ی متفاوتی می‌ده.»",
    "بعضی‌ها می‌گن بهترین جنس‌ها همیشه درست قبلِ رفرش ظاهر می‌شن.",
    "صدای چانه‌زنی از ته بازار میاد — انگار امروز کسی کوتاه نمیاد.",
    "یکی زمزمه کرد که کاروانِ سیاه هر از گاهی سر می‌زنه، ولی هیچ‌وقت خبر نمی‌ده کِی.",
]

# ─── 🌌 وقتی خودِ کاروان "نایاب" از آب دربیاد ─────────────────────────
ELITE_CARAVAN_BANNER = "🌌 **این یکی فرق داره.** تاجرِ مرموز امشب یه چیزی آورده که خودشم انگار مطمئن نیست باید بفروشدش یا نه."
ELITE_CARAVAN_LINE = "«این... این یکی رو خیلی وقته نگه داشته بودم. شاید وقتشه.»"

# ─── 🎁 جکپاتِ چانه‌زنی ────────────────────────────────────────────────
HAGGLE_JACKPOT_LINES = [
    "🎁 فروشنده یهو یه چیزِ اضافه انداخت تو کیسه‌ت: «اینم رو بدون‌حساب، وجدانم اذیتم کرد.»",
    "🎁 «بگیر، این‌و مفتی بردار. امروز روزِ خوبیه.»",
    "🎁 یه چیزِ کوچیک، بدونِ اینکه بفهمی کِی، بینِ خریدهات جا خوش کرد.",
]
...
def rumor_line() -> str:
    return _pick(HUB_RUMORS)


def jackpot_line() -> str:
    return _pick(HAGGLE_JACKPOT_LINES)


def time_left_str(expires_at: float) -> str:
    """فرمتِ خوانا برای زمانِ باقی‌مونده تا رفرشِ بعدی، مثلِ '۲ ساعت و ۱۵ دقیقه'."""
    secs = max(0, int(expires_at - time.time()))
...
def hype_tag(doc: dict) -> str:
    """برچسبِ داغیِ یه غرفه، بر اساسِ تعدادِ خریدِ کلِ بازیکن‌ها تو همین چرخه."""
    count = doc.get("purchase_count", 0)
    for threshold, label in HYPE_TIERS:
        if count >= threshold:
            return label
    return ""


def epic_reaction_line(item: dict) -> str | None:
    """اگه رریتیِ آیتم به‌قدرِ کافی بالا بود، یه واکنشِ خفنِ بازار برمی‌گردونه."""
    try:
        from item_system import RARITY_ORDER
        idx = RARITY_ORDER.index(item.get("rarity", "common")) if item.get("rarity") in RARITY_ORDER else 0
    except Exception:
        idx = 0
    if idx >= EPIC_REACTION_MIN_INDEX:
        return _pick(EPIC_PURCHASE_REACTIONS)
    return None


def rarity_dot(item: dict) -> str:
    """فقط رنگِ رریتی، برای نمایشِ فشرده کنارِ اسمِ آیتم تو دکمه‌ها.
    برای common چیزی نشون نمی‌ده تا شلوغ نشه."""
    try:
        from item_system import RARITY_DATA
        r = item.get("rarity")
        if r and r != "common":
            rd = RARITY_DATA.get(r)
            if rd:
                return f"{rd['emoji']} "
    except Exception:
        pass
...
def rarity_tag(item: dict) -> str:
    """برچسبِ کوتاهِ رریتی برای نمایشِ کنارِ اسمِ آیتم، مثلِ '🟠 میتیک'."""
    try:
        from item_system import RARITY_DATA
        r = item.get("rarity")
        rd = RARITY_DATA.get(r)
        if rd:
            return f"{rd['emoji']} {rd['label']}"
    except Exception:
        pass
    return ""
...
def _gen_stock(stall: dict, map_name: str, seed_t: float, price_mult: float = 1.0,
                special_chance_mult: float = 1.0) -> list[dict]:
    ...
    for _ in range(n):
        ...
        rows.append({
            ...
            "item": row["item"],
            "price": max(1, int(round(row["price"] * price_mult))),
            "qty_total": rng.randint(*STOCK_QTY_RANGE),
            "qty_left": None,
            "special": False,
        })

    if rng.random() < min(0.95, SPECIAL_ROW_CHANCE * special_chance_mult):
        srow = _gen_special_row(stall, map_name, rng)
        rows.append({
            "row_id": uuid.uuid4().hex[:8],
            "item": srow["item"],
            "price": max(1, int(round(srow["price"] * price_mult))),
            "qty_total": 1,
            "qty_left": None,
            "special": True,
        })
...
def get_stock(stall_id: str, map_name: str) -> dict:
    ...
    نبوده، یه چرخه‌ی تازه می‌سازه (با احتسابِ رویدادِ بازارِ همون نقشه).
    سندِ برگشتی مشترکِ بینِ همه‌ی بازیکن‌هاست."""
    ...
    if not doc or doc.get("expires_at", 0) <= now:
        ...
        event = get_market_event(map_name)
        edata = MARKET_EVENT_TYPES.get(event.get("type", "normal"), MARKET_EVENT_TYPES["normal"])
        stock = _gen_stock(
            stall, map_name, now,
            price_mult=edata["price_mult"],
            special_chance_mult=edata["special_chance_mult"],
        ) if stall else []
        doc = {
            "_id": stall_id,
            "map_name": map_name,
...
# ============================================================
#  🎪 مدیریتِ رویدادِ بازار + 🐫 کاروانِ سیاه
# ============================================================
def _weighted_choice(rng: random.Random, options: dict[str, dict]) -> str:
    keys = list(options.keys())
    weights = [options[k]["weight"] for k in keys]
    return rng.choices(keys, weights=weights, k=1)[0]


def _next_rarity(rarity: str, bump: int) -> str:
    try:
        from item_system import RARITY_ORDER
        idx = RARITY_ORDER.index(rarity) if rarity in RARITY_ORDER else 0
        idx = min(len(RARITY_ORDER) - 1, idx + bump)
        return RARITY_ORDER[idx]
    except Exception:
        return rarity


def _gen_caravan_item(map_name: str, rng: random.Random) -> dict:
    """کالای تک‌عددیِ تاجرِ مرموزِ کاروانِ سیاه — یه پله بالاترِ سقفِ
    رریتیِ عادیِ همون نقشه (یا حتی دو پله، اگه شانسِ نایاب بیاد)، از
    یکی از قالب‌های همون شهر (اگه بود)."""
    map_tier = _map_tier_of(map_name)
    rarity_pool = RARITY_POOL_BY_MAP_TIER.get(map_tier, ["rare"])
    is_elite = rng.random() < ELITE_CARAVAN_CHANCE
    bump = ELITE_CARAVAN_RARITY_BUMP if is_elite else CARAVAN_RARITY_BUMP
    top_rarity = _next_rarity(rarity_pool[-1], bump)
    price_mult = ELITE_CARAVAN_PRICE_MULT if is_elite else CARAVAN_PRICE_MULT

    gear_stalls = [s for s in get_stalls(map_name) if s.get("templates")]
    if gear_stalls:
        stall = rng.choice(gear_stalls)
        template = dict(rng.choice(stall["templates"]))
        import item_system as isy
        item = isy.generate_item(template, 40, forced_rarity=top_rarity, drop_source=f"citymarket_caravan:{map_name}")
        kind_mult = KIND_PRICE_MULT.get(stall["kind"], 3.0)
    else:
        from item_system import generate_consumable
        item = generate_consumable(player_level=rng.randint(40, 55))
        kind_mult = 2.0

    price = int(item.get("sell", 200) * kind_mult * price_mult)
    return {"item": item, "price": price, "elite": is_elite}


def get_market_event(map_name: str) -> dict:
    """رویدادِ فعلیِ بازارِ این نقشه رو برمی‌گردونه؛ منقضی که بشه یه
    رویدادِ تازه (seed‌دار و مشترک بینِ همه‌ی بازیکن‌ها) می‌سازه."""
    from database import city_market_col
    col = city_market_col()
    now = time.time()
    doc_id = f"cmkt_event:{map_name}"
    doc = col.find_one({"_id": doc_id})
    if not doc or doc.get("expires_at", 0) <= now:
        rng = _rng_for(f"event:{map_name}", now)
        etype = _weighted_choice(rng, MARKET_EVENT_TYPES)
        doc = {
            "_id": doc_id,
            "map_name": map_name,
            "type": etype,
        ...
        if etype in CARAVAN_EVENTS:
            caravan_rng = _rng_for(f"caravan:{map_name}", now)
            gen = _gen_caravan_item(map_name, caravan_rng)
            doc["caravan"] = {
                "item": gen["item"],
                "price": gen["price"],
                "qty_left": 1,
                "elite": gen.get("elite", False),
            }
        col.replace_one({"_id": doc_id}, doc, upsert=True)
    return doc


def event_banner(map_name: str) -> str | None:
    event = get_market_event(map_name)
    edata = MARKET_EVENT_TYPES.get(event.get("type", "normal"), MARKET_EVENT_TYPES["normal"])
    return edata.get("banner")


def event_label(map_name: str) -> str:
    event = get_market_event(map_name)
    edata = MARKET_EVENT_TYPES.get(event.get("type", "normal"), MARKET_EVENT_TYPES["normal"])
    return edata["label"]


def has_caravan(map_name: str) -> bool:
    event = get_market_event(map_name)
    return event.get("type") in CARAVAN_EVENTS and event.get("caravan", {}).get("qty_left", 0) > 0


def buy_caravan_item(map_name: str) -> dict | None:
    """خریدِ اتمیکِ کالای کاروانِ سیاه — اولین کسی که کلیک کنه می‌بره.
    اگه دیگه موجود نبود یا رویداد عوض شده باشه، None برمی‌گردونه."""
    from database import city_market_col
    col = city_market_col()
    doc_id = f"cmkt_event:{map_name}"
    result = col.find_one_and_update(
        {"_id": doc_id, "caravan.qty_left": {"$gt": 0}, "expires_at": {"$gt": time.time()}},
        {"$inc": {"caravan.qty_left": -1}},
    )
    if not result:
        return None
    caravan = result.get("caravan")
    if not caravan:
        return None
    return {"item": dict(caravan["item"]), "price": caravan["price"], "elite": caravan.get("elite", False)}
...
def buy_row(stall_id: str, map_name: str, row_id: str) -> dict | None:
    ...
    for row in doc.get("stock", []):
        if row["row_id"] == row_id:
            ...
            col.update_one(
                {"_id": stall_id},
                {"$set": {"stock": doc["stock"]}, "$inc": {"purchase_count": 1}},
            )
...
def try_haggle(player: dict, stall_id: str, map_name: str) -> dict:
    ...
    شخصیِ موقت (تا رفرشِ بعدی) رو این غرفه فعال می‌کنه؛ گاهی هم یه
    جکپاتِ کوچیک (یه آیتمِ رایگان) همراهش میاد."""
    stall = get_stall(stall_id)
    if not stall:
    ...
    if success:
        ...
        if random.random() < HAGGLE_JACKPOT_CHANCE:
            from item_system import generate_consumable
            jackpot_item = generate_consumable(player_level=random.randint(10, 40))
            player.setdefault("inventory", []).append(jackpot_item)

    return {
        "ok": True, "success": success, "chance": chance, "stall": stall,
        "tier": tier, "jackpot_item": jackpot_item,
    }
