...
def calc_combat(player: dict, enemy: dict, attack_type: str) -> dict:
    ...
    if player_class == "wizard":
        ...
        # ⚖️ بافِ جادوگر: پایه از ۰.۰۸/۰.۱۵ به ۰.۱۰/۰.۱۸ رفت بالا، به‌علاوه‌ی
        # بونوسِ تجمعیِ «تسلطِ آرکین» که با تعدادِ طلسمِ ترکیبی رشد می‌کنه
        # (موازیِ Bond ماجراجو) — class_abilities.wizard_mastery_bonus
        class_crit_add   = 0.10
        class_dmg_mult_add = 0.18
        try:
            from class_abilities import wizard_mastery_bonus
            _wm = wizard_mastery_bonus(player)
            class_crit_add += _wm["crit_add"]
            class_dmg_mult_add += _wm["dmg_mult_add"]
            class_lifesteal += _wm["lifesteal"]
        except Exception:
            pass
    ...
    # ─── 🏔 بونوسِ پسیوِ مسیرِ ارتقای کلاس (class_ascension_system.py) ──
    # اگه بازیکن یکی از دو مسیرِ ارتقای کلاسش رو گرفته باشه (از لولِ ۱۵
    # به بعد)، این بونوسِ دائمی همیشه فعاله — کنارِ بونوسِ خودِ کلاس جمع می‌شه.
    asc_defense_pct = 0.0
    try:
        from class_ascension_system import ascension_bonus
        _ab = ascension_bonus(player)
        class_dmg_mult_add += _ab.get("dmg_mult_add", 0.0)
        class_crit_add     += _ab.get("crit_add", 0.0)
        class_lifesteal    += _ab.get("lifesteal", 0.0)
        asc_defense_pct     = _ab.get("defense_pct", 0.0)
    except Exception as e:
        log_sync(f"⚠️ calc_combat: خطا تو بونوسِ ارتقای کلاس (uid={player.get('id')}): {type(e).__name__}: {e}", "ERROR")

    # ─── 🆕 آرتیفکتِ لول‌بندی‌شده‌ی کلاس (class_artifact_core.py) ─────
    # چوب‌دستیِ جادوگر / عصای تاجر / انگشترِ درمانگر — دقیقاً موازیِ
    # kcore کاتانا؛ dmg_mult‌ش روی katana_soul_dmg_mult می‌شینه (همون
    # متغیری که پایین‌تر تو raw ضرب می‌شه)، crit/lifesteal‌ش هم اضافه.
    acore = {"crit": 0, "lifesteal": 0, "dmg_mult": 1.0, "dmg_mult_flat": 0.0,
             "special": None, "special_active": False, "skills": {}}
    if player_class in ("wizard", "merchant", "healer"):
        try:
            ...
            katana_soul_dmg_mult = acore["dmg_mult"] + acore["dmg_mult_flat"]
        except Exception as e:
            log_sync(f"⚠️ calc_combat: خطا تو آرتیفکتِ کلاس (uid={player.get('id')}): {type(e).__name__}: {e}", "ERROR")
    ...
    # 🗡️ بونوسِ passiveِ کاتانای رلیکِ اکیپ‌شده (katana_relic_abilities.py) —
    # دقیقاً مثلِ ست‌ها/مُهرها/هانت‌کوئست‌لاین، قاطیِ setb می‌شه.
    try:
        from katana_relic_abilities import get_relic_passive_bonus_stats
        for k, v in get_relic_passive_bonus_stats(player).items():
            setb[k] = setb.get(k, 0) + v
    except Exception as e:
        if not isinstance(e, ImportError):
            log_sync(f"⚠️ calc_combat: خطا تو passiveِ کاتانای رلیک (uid={player.get('id')}): {type(e).__name__}: {e}", "ERROR")
    ...
    # پرکِ دائمیِ رشته‌های سرنوشت (fate_threads.py)
    try:
        from fate_threads import get_fate_bonus
        fate_dmg_pct = get_fate_bonus(player, "dmg_pct")
    except Exception as e:
        log_sync(f"⚠️ calc_combat: خطا تو fate_threads (uid={player.get('id')}): {type(e).__name__}: {e}", "ERROR")
        fate_dmg_pct = 0

    # یارِ نبردِ همدم‌ها (companion_gacha_handlers.py)
    try:
        from companion_gacha_handlers import get_battle_partner_combat_bonus
        companion_dmg_pct = get_battle_partner_combat_bonus(player)
    except Exception as e:
        log_sync(f"⚠️ calc_combat: خطا تو companion_gacha_handlers (uid={player.get('id')}): {type(e).__name__}: {e}", "ERROR")
        companion_dmg_pct = 0
    ...
    try:
        ...
        _pulse_crit = 0
    dmg_mult    = atk["dmg_mult"] + skb["dmg_pct"] + setb.get("dmg_pct", 0) + rb["dmg_pct"] + guild_dmg_pct + fate_dmg_pct + companion_dmg_pct + class_dmg_mult_add
    ...
    if enemy_survives and not status_blocks_counter and random.random() < max(0.05, retaliate_chance):
        ...
        base_enemy_dmg = int(base_enemy_dmg * (1 - skb["counter_dmg_reduction"] - setb.get("defense_pct", 0) - relic_defense_pct - asc_defense_pct))
        try:
    ...
    # 🐛 فیکس: این چک قبلاً enemy_survives رو نگاه نمی‌کرد — یعنی حتی رو
    # ضربه‌ی کشنده (که همون لحظه enemy["hp"] رو به صفر می‌رسوند) بازم
    # خارها دمیج برمی‌گردوندن. الان مثلِ ضدحمله‌ی معمولی، فقط وقتی دشمن
    # از این ضربه جون سالم به در ببره فعال می‌شه.
    if enemy_survives:
    ...
    # ─── 🆕 اثر ویژه‌ی تایرِ آرتیفکتِ کلاس (Legendary/Mythic) ────────
    result["artifact_bonus_dmg"] = 0
    if acore["special"] in ("mana_overload", "golden_strike", "grace_of_light") and acore["special_active"] and result["dmg"] > 0:
        if random.random() < 0.15:
            extra = result["dmg"]
            result["dmg"] += extra
            result["artifact_bonus_dmg"] = extra
            try:
                from class_artifact_core import SPECIAL_INFO
                logs.append(f"🌟 **{SPECIAL_INFO[acore['special']]['name_fa']}!** +{extra} آسیب اضافه")
            except Exception:
                pass
            if acore["special"] == "golden_strike":
                gold_bonus = max(5, int(extra * 0.5))
                player["zen"] = player.get("zen", 0) + gold_bonus
                logs.append(f"💰 +{gold_bonus} Zen غنیمتِ فوری")
            elif acore["special"] == "grace_of_light":
                heal = int(extra * 0.5)
                if heal > 0:
                    result["lifesteal_heal"] = result.get("lifesteal_heal", 0) + heal
                    logs.append(f"💧 +{heal} HP از فیضِ نور")
    elif acore["special"] in ("arcane_singularity", "market_domination", "divine_ascension") and acore["special_active"] and result["dmg"] > 0 and not enemy_survives:
        if acore["special"] == "arcane_singularity":
            csd = player.setdefault("class_system_data", {})
            gain = max(1, int(enemy_hp_before * 0.10))
            csd["mana"] = min(csd.get("max_mana", 50), csd.get("mana", 0) + gain)
            logs.append(f"🔮 **تکینگیِ آرکین (Mythic)!** با کشتنِ دشمن +{gain} مانا گرفتی")
        elif acore["special"] == "market_domination":
            gain = max(1, int(enemy_hp_before * 0.10))
            player["zen"] = player.get("zen", 0) + gain
            logs.append(f"💰 **سلطه‌ی بازار (Mythic)!** با کشتنِ دشمن +{gain} Zen گرفتی")
        elif acore["special"] == "divine_ascension":
            heal = max(1, int(enemy_hp_before * 0.10))
            result["lifesteal_heal"] = result.get("lifesteal_heal", 0) + heal
            logs.append(f"✨ **عروجِ الهی (Mythic)!** با کشتنِ دشمن +{heal} HP گرفتی")

    # ─── ابیلیتیِ یکتای کاتانای رلیک (katana_relic_abilities.py) ──
    # فقط اگه سلاحِ اکیپ‌شده یکی از کاتاناهای گردونه (katana_wheel_system)
    # باشه اثر می‌ذاره؛ برای بقیه‌ی کاراکترها/سلاح‌ها کاملاً بی‌اثره.
    try:
        from katana_relic_abilities import roll_relic_proc
        relic_log = roll_relic_proc(player, enemy, result)
        if relic_log:
            logs.append(relic_log)
    except Exception as e:
        if not isinstance(e, ImportError):
            log_sync(f"⚠️ calc_combat: خطا تو ابیلیتیِ کاتانای رلیک (uid={player.get('id')}): {type(e).__name__}: {e}", "ERROR")
...
def get_combat_stats_summary(player: dict) -> dict:
    ...
    total_dmg_pct    = (atk["dmg_mult"] - 1.0) + skb.get("dmg_pct", 0) + setb.get("dmg_pct", 0) + rb.get("dmg_pct", 0) + guild_dmg_pct + fate_dmg_pct + companion_dmg_pct + class_dmg_mult_add
    ...
    total_defense    = setb.get("defense_pct", 0) + relic_defense_pct + asc_defense_pct
