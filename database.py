...
def city_market_col() -> Collection:
    """غرفه‌های زنده‌ی بازارِ محلیِ هر شهر (میوه/غذا/لباس/...) — یه سند به‌ازای هر غرفه."""
    return get_db()["city_market_stalls"]
...
_NEW_FIELD_DEFAULTS = {
    ...
    # ═══ سیستمِ دوستی — friend_system.py / friend_handlers.py ═══════════
    "friends":              lambda: [],   # لیستِ uidِ دوستانِ تایید‌شده
    "friend_requests_in":   lambda: [],   # درخواست‌های دریافتی (uid)
    "friend_requests_out":  lambda: [],   # درخواست‌های ارسالی (uid)
    "friend_gift_sent_at":  lambda: {},   # {uid_str: timestamp} — آخرین هدیه‌ای که به این دوست دادی
    "friend_gifts_sent_total": lambda: 0, # آمارِ فلیوری تو پروفایل/مقایسه

    # ═══ چالشِ رقابتیِ هفتگی — weekly_challenge.py ══════════════════════
    "weekly_challenge_snapshot": lambda: 0,   # مقدارِ متریکِ چالش در لحظه‌ی شروعِ هفته‌ی جاری
    "weekly_challenge_week_id":  lambda: "",  # کدومِ هفته این snapshot براشه (idempotency)
}
...
def _sync_pending_levelups(player: dict) -> bool:
    ...
    if leveled:
        try:
            ...
            pass
        try:
            from constellation_system import grant_core_points
            grant_core_points(player, old_level, player["level"])
        except ImportError:
            pass
    return leveled
