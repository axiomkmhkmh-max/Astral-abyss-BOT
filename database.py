# ============================================================
#  ASTRAL ABYSS RPG — Database (PostgreSQL / Supabase)
# ------------------------------------------------------------
#  قبلاً این فایل مستقیم به MongoDB وصل می‌شد. حالا از pg_shim.py
#  استفاده می‌کنه که دقیقاً همون API قدیمیِ pymongo رو شبیه‌سازی
#  می‌کنه (find_one/find/update_one/...) ولی پشتش PostgreSQL
#  (Supabase) با ذخیره‌سازیِ JSONB هست. به همین خاطر بقیه‌ی توابعِ
#  این فایل (players_col, boss_col, ...) و ۳ فایلِ دیگه‌ای که
#  مستقیم get_db() رو صدا می‌زنن (region_boss_system.py,
...
from pg_shim import Collection, get_shim_db
