...
async def cb_graid_hit(event):
    ...
    if result["boss_killed"]:
        ...
        for ruid, r in rewards.items():
            # 🔒 باگ‌فیکس: همون باگِ گم‌شدنِ لوت (مثلِ داسِ روحِ تاناتپ) که تو
            # region_boss_handlers فیکس شد — رِیدِ گروهی هم بدونِ player_lock
            # بود، درحالی‌که چندنفره‌ست و دقیقاً همون ریسک رو داره.
        ...
        # 🏆 امتیازِ رقابتِ هفتگیِ گروه‌ها (group_play.py)
        try:
            import group_play
            _chat = event.message.chat if is_cb else event.chat
            await group_play.on_boss_killed(chat_id, len(boss.get("contributors", {})), _chat.title)
        except Exception:
            pass
