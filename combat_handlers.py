...
async def resolve_hit(cb: CallbackQuery, player: dict, atk_type: str):
    ...
    # 🐛 فیکس: قبلاً فقط بازیکن‌بودن‌زنده چک می‌شد، نه دشمن — یه دشمنِ
    # مرده (که همین ضربه کشتش) هنوز می‌تونست «ضربه‌ی مرگبار» بزنه.
    ...
    if killed:
        ...
        # مرحله‌ی ۳ با هر کشتن (وایرِ /attack؛ /loot از mob_combat.py) ───
    ...
    if player["hp"] <= 0 and not revived:
        ...
        # ─── 🆕 آرتیفکتِ کلاس: اثر مرگ روی پیوند (class_artifact_core.py) ───
        if player.get("class") in ("wizard", "merchant", "healer"):
            try:
                from class_artifact_core import apply_artifact_death_penalty, artifact_talk
                _art_death = apply_artifact_death_penalty(player)
                if _art_death.get("message"):
                    katana_death_msg = f"💫 «{artifact_talk(player, 'death')}»\n{_art_death['message']}"
            except Exception:
                pass
    ...
    if tutorial_graduated:
        ...
        await cb.message.answer("🔓 همه‌چی باز شد — این کلمه‌ها رو تایپ کن:\n\n" + main_menu_text(is_group=is_group, player=player))
    elif tutorial_loot_step:
        await cb.message.answer("🗺 حالا می‌تونی «لوت» رو هم تایپ کنی!")
