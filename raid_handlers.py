...
from mob_combat import dungeon_state  # چک می‌کنیم آیا بازیکن داخل زنجیره‌ی دانجن هست یا نه
...
async def handle_raid_outcome(cb: CallbackQuery):
    ...
    # 🐛 باگ‌فیکس: دکمه‌ی «فروش» بعدِ این رویداد قبلاً کلِ کوله‌پشتی رو
    # می‌فروخت (حتی آیتم‌های قفل‌شده‌ی قبلی)، نه فقط چیزی که همین الان
    # دراپ شده. اول نشانه‌ی «تازه‌لوت‌شده»یِ باقی‌مونده از دورِ قبل رو پاک
    # می‌کنیم (اگه بازیکن قبلاً نه فروخته نه صریحاً نگه داشته)، بعد فقط
    # آیتم‌های همین دراپ رو نشونه‌گذاری می‌کنیم — cb_raidloot_sell_new
    # پایین فقط همینا رو می‌فروشه.
    ...
    # Level up check
    # 🐛 باگ‌فیکس: قبلاً این حلقه اصلاً سقفِ سطح رو چک نمی‌کرد (برخلافِ
    # combat_handlers.py) — یعنی XP زیاد تو ریید می‌تونست از effective_max_level
    # رد بشه. هم‌چنین +۱۰ HP در هر لول با +۵ باقیِ بازی (combat/mob_combat) ناهماهنگ بود.
    leveled = False
    ...
    if in_dungeon:
        ...
        if d_state["stage"] >= d_state["total"]:
            ...
            kb = InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="➡️ ادامه‌ی دانجن", callback_data=f"dg_next:{map_name}:{uid}", style=ButtonStyle.PRIMARY),
            ],[
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text=f"💰 فروشِ همین لوت ({bz_to_display(total_val)})", callback_data="raidloot:sell_new", style=ButtonStyle.DANGER),
        InlineKeyboardButton(text="🎒 نگه دار", callback_data="loot:keep", style=ButtonStyle.PRIMARY),
    ],[
...
async def cb_raidloot_sell_new(cb: CallbackQuery):
    ...
    to_sell_ids = {id(it) for it in to_sell}  # 🆔 حذف بر اساسِ identity، نه برابریِ محتوا (ضدِ باگِ آیتم‌های تکراری)
...
def register_raid_handlers(dp: Dispatcher):
    dp.callback_query.register(
        handle_raid_outcome,
        F.data.startswith("raid:")
    )
    dp.callback_query.register(cb_dungeon_next, F.data.startswith("dg_next:"))
    dp.callback_query.register(cb_raidloot_sell_new, F.data == "raidloot:sell_new")
