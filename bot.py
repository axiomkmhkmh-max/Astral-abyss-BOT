...
async def main():
    ...
    # ─── 🔁 حلقه‌ی ویروسی: /invite + پاداشِ فعال‌سازیِ دعوت‌ها ────────
    # باید قبلِ dp.message.register(lore_chat) بیاد (اون همه‌چی رو می‌گیره).
    from viral_loop_handlers import register_viral_loop_handlers
    register_viral_loop_handlers(dp, bot)

    # ─── 🔥 نگه‌داشتنِ اجتماعی: /streaks + ردیابیِ فعالیت (middleware) ──
    from social_retention_handlers import register_social_retention_handlers
    register_social_retention_handlers(dp, bot)
