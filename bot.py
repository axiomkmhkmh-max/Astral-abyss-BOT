...
GROUP_COMMANDS = [
    ...
    BotCommand(command="classascend", description="🏔 ارتقای کلاس (از لولِ ۱۵)"),
    BotCommand(command="friends", description="👥 لیستِ دوستان"),
    BotCommand(command="invite", description="🎁 دعوتِ دوستان + پاداش"),
    BotCommand(command="streaks", description="🔥 استریکِ دوتایی با دوستان"),
    BotCommand(command="groups", description="🏆 رقابتِ هفتگیِ گروه‌ها"),
    BotCommand(command="addfriend", description="➕ افزودنِ دوست"),
    BotCommand(command="weekly", description="🏁 چالشِ رقابتیِ هفته"),
...
PRIVATE_COMMANDS = GROUP_COMMANDS + [
    ...
    BotCommand(command="core", description="🌌 صورت‌فلکی (هسته‌ی زنده)"),
]
...
# ─── گیتِ سراسریِ «فقط توی گروه» ──────────────────────────────────
# از این به بعد پنل/کلماتِ بازی (حمله، وضعیت، دسته‌ها، ...) فقط توی
# گروه کار می‌کنن؛ توی پیوی بلاک می‌شن و به گروه ارجاع داده می‌شن.
# باید همین اولِ کار (قبل از بقیه‌ی middlewareها) ثبت بشه تا هیچ
# هندلری توی پیوی اجرا نشه.
from group_only_gate import group_only_gate_middleware
dp.message.middleware()(group_only_gate_middleware)
...
CATEGORIES: dict[str, dict] = {
    "ماجراجو": {
        ...
        "buttons": [
            ...
            "⚜️ قدرت‌های کلاس", "🏔 ارتقای کلاس", "PvP", "🏟 آرنا", "رده‌بندی", "ایونت", "🌀 شکاف Abyss",
    ...
    "تاجر": {
        ...
        "buttons": ["🚶 سفر", "🤝 معامله‌ی روزانه", "مغازه‌ی من", "بازار سیاه", "🦯 عصا", "⚜️ قدرت‌های کلاس", "🏔 ارتقای کلاس"],
    },
    "درمانگر": {
        "title": "✨ *درمانگر*",
        "class_only": CLASS_HEALER,
        "buttons": ["بیمارستان", "🩺 نوبت‌دهی", "💊 درمان", "💍 انگشتر", "⚜️ قدرت‌های کلاس", "🏔 ارتقای کلاس"],
    },
    "جادوگر": {
        "title": "🧙 *جادوگر*",
        "class_only": CLASS_WIZARD,
        "buttons": ["🛠 کارگاه", "🔮 مشتری‌ها", "🪄 چوب‌دستی", "⚜️ قدرت‌های کلاس", "🏔 ارتقای کلاس"],
    },
    "گیلد": {
        "title": "🏰 *گیلد*",
        "class_only": None,
    ...
    "من": {
        "title": "📊 *شخصیتِ من*",
        "class_only": None,
        "buttons": ["وضعیت", "کوله‌پشتی", "🎽 تجهیزات", "🎴 کارت من", "📖 کدکس", "مهارت‌ها", "🌌 صورت‌فلکی", "دستاوردها", "تیر جهان", "🧵 رشته‌های سرنوشت", "🔗 اتصال حساب"],
    },
    "اجتماعی": {
        "title": "🏛 *اجتماعی*",
        "class_only": None,
        "buttons": ["تیم", "معامله", "ردیابی", "👥 دوستان", "🏁 چالش هفته"],
    },
    "اقتصاد": {
        "title": "💰 *اقتصاد و بازار*",
        "class_only": None,
        "buttons": ["حراجی", "کازینو", "📜 تابلوی کارگزار", "بانک", "🎡 چرخِ شانس"],
    },
    "اطلاعات": {
        "title": "📖 *اطلاعات*",
        "class_only": None,
...
LEVEL_REQUIREMENTS: dict[str, int] = {
    ...
    "⚜️ قدرت‌های کلاس": 1, "🏔 ارتقای کلاس": 1, "PvP": 5, "🏟 آرنا": 5, "رده‌بندی": 1, "🏁 چالش هفته": 1, "ایونت": 1, "🌀 شکاف Abyss": 15,
    "🐎 مونت‌ها": 1, "🌌 هم‌گرایی": 1, "🕊 الهه": 1,
    "🚶 سفر": 1, "🤝 معامله‌ی روزانه": 1, "مغازه‌ی من": 12, "بازار سیاه": 1, "🦯 عصا": 1,
    "بیمارستان": 1, "🩺 نوبت‌دهی": 1, "💊 درمان": 1, "💍 انگشتر": 1,
    "🛠 کارگاه": 1, "🔮 مشتری‌ها": 1, "🪄 چوب‌دستی": 1,
    "گیلدها": 10,
    "وضعیت": 1, "کوله‌پشتی": 1, "🎽 تجهیزات": 1, "🎴 کارت من": 1, "📖 کدکس": 1, "مهارت‌ها": 1, "🌌 صورت‌فلکی": 1, "دستاوردها": 1, "تیر جهان": 1, "🧵 رشته‌های سرنوشت": 1, "🔗 اتصال حساب": 1,
    "تیم": 5, "معامله": 10, "ردیابی": 5,
    "حراجی": 12, "کازینو": 10, "📜 تابلوی کارگزار": 12, "بانک": 1, "🎡 چرخِ شانس": 1,
    "کدکس": 1, "پس نبرد": 1,
...
def main_menu_text(is_group: bool = False, story_badge: bool = False, player: dict | None = None) -> str:
    """🆕 لیستِ متنیِ شورتکات‌ها و دسته‌ها — جایگزینِ پنلِ دکمه‌ایِ قدیمی.
    دیگه هیچ کیبوردی فرستاده نمی‌شه؛ کاربر همین کلمه‌ها رو مستقیم
    تایپ می‌کنه (دقیقاً همون هندلرهای متنیِ قبلی جوابشون رو می‌دن)."""
    ...
    lines = ["⌨️ به‌جای دکمه، همینا رو تایپ کن:", "• " + "  •  ".join(shortcuts)]
    if cat_names:
        lines.append("")
        lines.append("📂 دسته‌ها (بزن/تایپ کن تا زیرمجموعه‌شو ببینی):")
        lines.append("• " + "  •  ".join(cat_names))
    return "\n".join(lines)


def main_kb(is_group: bool = False, story_badge: bool = False, player: dict | None = None):
    """🆕 دیگه هیچ پنلِ کیبوردی (Reply Keyboard) فرستاده نمی‌شه — نه تو
    پیوی، نه تو گروه. همه‌چی با تایپِ کلمه/کامند انجام می‌شه. این تابع
    فقط برای سازگاری با صدازننده‌های قدیمی نگه داشته شده و همیشه
    ReplyKeyboardRemove برمی‌گردونه؛ لیستِ کلمه‌ها رو از main_menu_text
    بگیر و توی متنِ پیام بذار."""
...
def category_menu_text(category_key: str, player_level: int) -> str:
    """🆕 لیستِ متنیِ آیتم‌های یه دسته — جایگزینِ زیرپنلِ دکمه‌ایِ قدیمی.
    آیتم‌های قفل (زیرِ سطحِ لازم) با 🔒 و سطحِ لازم مشخص می‌شن؛ بقیه رو
    می‌شه مستقیم تایپ کرد."""
    ...
    for b in buttons:
        ...
        if player_level >= req:
            ...
            lines.append(f"🔒 {b} (نیاز به سطح {req})")
    lines.append("")
    lines.append(f"↩️ برای برگشتن، بنویس: «{BACK_TO_MAIN}»")
    return "\n".join(lines)


def category_kb(category_key: str, player_level: int):
    """🆕 دیگه زیرپنلِ کیبوردی فرستاده نمی‌شه. برای سازگاری با
    صدازننده‌های قدیمی نگه داشته شده — همیشه ReplyKeyboardRemove
    برمی‌گردونه؛ لیستِ آیتم‌ها رو از category_menu_text بگیر."""
    return ReplyKeyboardRemove()
...
async def cmd_category_menu(msg: Message, state: FSMContext):
    ...
    if only is not None and only != player.get("class"):
        # این دسته مخصوصِ یه کلاسِ دیگه‌ست — لیستِ متنیِ پنلِ درستِ خودش رو نشون می‌دیم
        is_group = _is_group_chat(msg.chat.type)
        text = "🔒 این دسته مخصوصِ کلاسِ دیگه‌ایه — این‌جا دسته‌های خودتُ داری:\n\n" + main_menu_text(is_group=is_group, player=player)
        await msg.reply(text)
        return
    text = cat["title"] + "\n\n" + category_menu_text(msg.text, player.get("level", 1))
    await msg.reply(text)

@dp.message(F.text.in_({BACK_TO_MAIN, "بازگشت", "منو"}))
async def cmd_back_to_main(msg: Message, state: FSMContext):
    ...
    text = "🌑 پنل اصلی:" + _story_panel_reminder(player) + "\n\n" + main_menu_text(is_group=is_group, story_badge=badge, player=player)
    await msg.reply(text)
...
async def cmd_start(msg: Message, state: FSMContext):
    ...
    BOT_LINK = "https://t.me/ASTRALABYSS_BOT"
    ...
    if not player:
        if is_group:
            await msg.answer(
                ...
                f"برو به [@ASTRALABYSS_BOT]({BOT_LINK}) پیام بده، اونجا /start رو بزن و "
        ...
        try:
            ...
            if ref and ref["kind"] == "group":
                ...
                # 🆕 پاداشِ رفرالِ شخصی: کسی که از رو کارت/دوئلِ خودش دعوت کرده،
                # واقعاً یه چیزی بگیره — وگرنه هیچ انگیزه‌ای برای شیرکردنِ لینک نیست.
                REFERRAL_ZEN_REWARD = 300
                REFERRAL_XP_REWARD = 20
                try:
                    referrer = await aget_player(ref["source"])
                    if referrer:
                        referrer["zen"] = referrer.get("zen", 0) + REFERRAL_ZEN_REWARD
                        referrer["xp"] = referrer.get("xp", 0) + REFERRAL_XP_REWARD
                        referrer["referral_count"] = referrer.get("referral_count", 0) + 1
                        await asave_player(ref["source"], referrer)
                        try:
                            await bot.send_message(
                                ref["source"],
                                f"🎉 یکی از طریقِ لینکِ تو به Astral Abyss ملحق شد!\n"
                                f"💰 +{REFERRAL_ZEN_REWARD} Zen | ✨ +{REFERRAL_XP_REWARD} XP\n"
                                f"👥 تعدادِ کلِ دعوت‌هات: {referrer['referral_count']}",
                            )
                        except Exception:
                            pass  # شاید کاربر ربات رو بلاک کرده باشه
                except Exception as e:
                    log_sync(f"🔴 referral reward error: {e}", "ERROR")
        except Exception as e:
    ...
    if not player.get("class"):
        if is_group:
            await msg.answer(
                ...
                f"برو به [@ASTRALABYSS_BOT]({BOT_LINK}) پیام بده و کاراکترت رو بساز — "
    ...
    if onboarding.is_in_tutorial(player):
        ...
        # 🆕 بازی فقط تو گروه پلی می‌شه — توی پیوی دیگه پنل نشون داده نمی‌شه
        from group_only_gate import GROUP_ONLY_TEXT
        await msg.reply(
            f"🌑 *بازگشتی، {player['name']}...*\nآبیس منتظرت بود.{extra}\n\n{GROUP_ONLY_TEXT}",
            reply_markup=ReplyKeyboardRemove(),
        )
    else:
        _badge = _story_badge_for(player)
        await msg.reply(
            f"🌑 *بازگشتی، {player['name']}...*\nآبیس منتظرت بود.{extra}{_story_panel_reminder(player)}\n\n"
            + main_menu_text(is_group=is_group, story_badge=_badge, player=player)
        )
    if login_msg:
        try:
            import tempfile, os
...
async def show_inventory_page(msg, uid: int, inv: list, page: int = 0, edit: bool = True):
    ...
    for gi, g in enumerate(page_groups):
        ...
        # 🐛 فیکس: صندوق‌ها/گاوصندوق‌ها (type=="lockbox") قبلاً هیچ دکمه‌ای
        # تو خودِ کوله‌پشتی نداشتن — فقط از یه صفحه‌ی جداگونه (بازار سیاه ←
        # Vax'ar Vault) قابل بازکردن بودن که خیلی کسی پیداش نمی‌کرد. حالا
        # اگه کلیدِ مناسبش تو کوله باشه، همین‌جا هم دکمه‌ی «باز کردن» میاد.
        if item.get("type") == "lockbox":
            try:
                from loot_engine import LOCKBOXES, KEYS
                need_key_id = LOCKBOXES.get(item.get("box_id"), {}).get("key")
                need_key_name = KEYS.get(need_key_id, {}).get("name", "?")
                has_key = any(
                    it.get("type") == "key" and it.get("key_id") == need_key_id
                    for it in inv
                )
            except ImportError:
                has_key = False
            ...
            if has_key:
                row.append(InlineKeyboardButton(
                    text="🔓 باز کردن",
                    callback_data=f"lbx_open:{real_idx}"
                , style=ButtonStyle.SUCCESS))
            else:
                # 🐛 فیکس: قبلاً فقط می‌گفت «نیاز به کلید» بدون گفتنِ اینکه
                # دقیقاً کدوم کلید — چون هر صندوق فقط با کلیدِ هم‌تایه‌ی
                # خودش باز می‌شه (نه هر کلیدی)، این ابهام باعث می‌شد بازیکن
                # فکر کنه کلید داره ولی درواقع کلیدِ تایرِ اشتباه رو داشته.
                row.append(InlineKeyboardButton(
                    text=f"🔑 نیاز به {need_key_name}",
                    callback_data="bm:vault"
...
async def cb_inv_use(cb: CallbackQuery):
    ...
    # ─── لقب‌های کازمتیکِ باکسِ شانسی (title_ember, title_dragon, ...) ────
    # 🐛 فیکس: قبلاً این آیتم‌ها فقط تو کوله‌پشتی می‌موندن، فعال‌سازی نداشتن
    # و تو صفحه‌ی لقب‌ها (/titles) هیچ‌وقت ظاهر نمی‌شدن.
    if item.get("title_reward"):
        title_name = item["title_reward"]
        ...
        await asave_player(uid, player)
        log_sync(
            f"🏅 **TITLE ACTIVATED** | {player.get('name','—')} (`{uid}`)\n"
            f"📦 {title_name}",
            "ECONOMY"
        )
        await cb.answer(f"🏅 لقب «{title_name}» باز شد! از /titles می‌تونی فعالش کنی.", show_alert=True)
        if inv:
            new_groups = group_inventory(inv)
            page = min(page, max(0, (len(new_groups) - 1) // 8))
            await show_inventory_page(cb.message, uid, inv, page)
...
LEADERBOARD_CATEGORIES = {
    ...
    "weekly": {
        "title": "🏁 چالشِ رقابتیِ این هفته",
        "key": lambda p: _weekly_challenge_score(p),
        "line": lambda p: f"🏁 {_weekly_challenge_score(p):,} امتیاز | Lv.{p.get('level',1)}",
    },
    "stand": {
...
def _weekly_challenge_score(player: dict) -> int:
    try:
        from weekly_challenge import challenge_score
        return int(challenge_score(player))
    except Exception:
        return 0
...
def _leaderboard_kb(active: str) -> InlineKeyboardMarkup:
    labels = {"level": "📊 سطح", "zen": "💰 ثروت", "pvp": "PvP", "weekly": "🏁 چالشِ هفته", "stand": "👻 استند"}
    row = []
...
# ─── 🏁 چالشِ رقابتیِ هفتگی (weekly_challenge.py) ──────────────
@dp.message(F.text == "🏁 چالش هفته")
@dp.message(Command("weekly"))
async def cmd_weekly_challenge(msg: Message):
    update_last_seen(msg.from_user.id)
    uid = msg.from_user.id
    player = await aget_player(uid)
    if not player or not player.get("class"):
        await msg.answer("❌ اول باید کاراکترت رو بسازی! /start رو بزن.")
        return
    from weekly_challenge import ensure_baseline, player_progress_text
    if ensure_baseline(player):
        await asave_player(uid, player)
    text = player_progress_text(player) + "\n\n" + _leaderboard_text("weekly")
    await msg.answer(text, reply_markup=_leaderboard_kb("weekly"))
...
async def btn_link_account(msg: Message):
    ...
    if not await aget_player(uid):
        return
    await msg.answer(await bridge.account_link.link_status_text(uid))
...
async def cmd_link(msg: Message):
    ...
    if len(parts) < 2:
        ok, result = await bridge.account_link.generate_link_code(uid)
        if not ok:
...
# ─── 🆕 دکمه‌های آرتیفکتِ کلاس (class_artifact_core.py) ──────────
@dp.message(F.text == "🪄 چوب‌دستی")
async def btn_staff(msg: Message):
    update_last_seen(msg.from_user.id)
    from class_artifact_handlers import cmd_staff
    await cmd_staff(msg)

@dp.message(F.text == "🦯 عصا")
async def btn_cane(msg: Message):
    update_last_seen(msg.from_user.id)
    from class_artifact_handlers import cmd_cane
    await cmd_cane(msg)

@dp.message(F.text == "💍 انگشتر")
async def btn_ring(msg: Message):
    update_last_seen(msg.from_user.id)
    from class_artifact_handlers import cmd_ring
    await cmd_ring(msg)
...
async def btn_hospital_legacy(msg: Message):
    update_last_seen(msg.from_user.id)
    from hospital_handlers import cmd_hospital
    _p = await aget_player(msg.from_user.id)
    await msg.answer("🏥 این بخش الان اسمش «بیمارستان» شده.\n\n" + main_menu_text(is_group=_is_group_chat(msg.chat.type), player=_p))
    await cmd_hospital(msg)
...
} | set(CATEGORIES.keys()) | set(LEVEL_REQUIREMENTS.keys()) | set(SHORTCUT_BUTTONS) | set(GROUP_SHORTCUT_BUTTONS) | {BACK_TO_MAIN, "بازگشت", "منو"}
...
async def main():
    set_bot(bot)
    from social_feed import set_bot as set_feed_bot
    set_feed_bot(bot)
    ...
    from constellation_handlers import register_core_handlers
    ...
    from class_artifact_handlers import register_class_artifact_handlers
    ...
    # 🆕 سیستمِ ارتقای کلاس (مسیرهای پیشرفتِ مخصوصِ هرکلاس، از لولِ ۱۵)
    from class_ascension_handlers import register_class_ascension_handlers
    # 🆕 سیستمِ دوستی (لیست/درخواست/هدیه/مقایسه) — friend_system.py
    from friend_handlers import register_friend_handlers
    ...
    # 🆕 پادشاهانِ نقشه‌ها — NPC ثابت و باحرف‌زدنی برای هر نقشه/شهر
    from king_handlers import register_king_handlers
    # 🆕 بازارهای زنده‌ی شهر — غرفه‌های میوه/غذا/لباس/... با موجودیِ مشترک
    from city_market_handlers import register_city_market_handlers
    # 🆕 چرخِ شانسِ روزانه — اسپینِ رایگانِ روزانه با استریک و جکپات
    from daily_wheel_handlers import register_daily_wheel_handlers
    ...
    register_king_handlers(dp, bot)
    register_city_market_handlers(dp, bot)
    register_daily_wheel_handlers(dp, bot)
    ...
    register_class_ascension_handlers(dp, bot)
    register_friend_handlers(dp, bot)
    ...
    register_class_artifact_handlers(dp, bot)
    ...
    from fate_threads_handlers import register_fate_handlers
    register_fate_handlers(dp, bot)
    ...
    from katana_wheel_handlers import register_katana_wheel_handlers
    register_katana_wheel_handlers(dp, bot)
    ...
    # 🆕 شکار با کلمه — فقط تو گروه‌ها: نوشتنِ «شکار»/«حمله» یه XP/Zen
    # کوچیک می‌ده. باید قبلِ register_group_handlers ثبت بشه، چون
    # handle_group_mention (تهِ اون تابع) روی *هر* پیامِ گروهی رجیستر
    # می‌شه و اگه زودتر بیاد، جلوی این فیلترِ متنیِ دقیق‌تر رو می‌گیره.
    from word_hunt_handlers import register_word_hunt_handlers
    register_word_hunt_handlers(dp, bot)

    # 🆕 همدم‌ها — گاچای کاراکتر با عکسِ AI رایگان (/claim, /collection, /favorite)
    ...
    # ─── 🆕 رشد/نگه‌داشت/بازیِ گروهی — ⚠️ این سه تا باید *قبلِ*
    # register_group_handlers بیان: اون یه catch-all برای همه‌ی پیام‌های
    # گروهیه و هر کامندی که بعدش ثبت بشه (مثلِ /invite تو گروه) رو می‌بلعه.
    from viral_loop_handlers import register_viral_loop_handlers
    register_viral_loop_handlers(dp, bot)                     # /invite /invitetop

    from social_retention_handlers import register_social_retention_handlers
    register_social_retention_handlers(dp, bot)               # /streaks + ردیابیِ فعالیت

    from group_play_handlers import register_group_play_handlers
    register_group_play_handlers(dp, bot)                     # /groups /gchest + صندوقِ گنج
    ...
    # ─── اینلاین‌مود (رشدِ ویروسی — @ASTRALABYSS_BOT تو هر چتی) ─────
    ...
    # ─── 🔁 چکِ فعال‌سازیِ دعوت‌ها (لِوِل ۵ → پاداشِ دعوت‌کننده + دعوت‌شده) ──
    from viral_loop import viral_loop_loop
    _spawn_task(viral_loop_loop(bot))

    # ─── ⏰ یادآوریِ استریکِ در خطر + پیامِ دلتنگی (روزی یه‌بار) ─────────
    from social_retention import retention_loop
    _spawn_task(retention_loop(bot))

    # ─── 🏆 جایزه‌ی هفتگیِ گروه‌ها (رقابتِ /groups) ───────────────────
    from group_play import group_play_loop
    _spawn_task(group_play_loop(bot))
