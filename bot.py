...
async def show_inventory_page(msg, uid: int, inv: list, page: int = 0, edit: bool = True):
    ...
    for gi, g in enumerate(page_groups):
        ...
        if item.get("type") == "lockbox":
            try:
                from loot_engine import LOCKBOXES
                need_key = LOCKBOXES.get(item.get("box_id"), {}).get("key")
                has_key = any(
                    it.get("type") == "key" and it.get("key_id") == need_key
                    for it in inv
                )
            except ImportError:
                has_key = False
            if has_key:
                ...
                row.append(InlineKeyboardButton(
                    text="🔑 نیاز به کلید (🗝️ بازار سیاه)",
