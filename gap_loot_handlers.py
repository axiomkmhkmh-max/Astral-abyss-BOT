...
def back_kb(cb_data: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="🔙 برگشت", callback_data=cb_data)
    ]])
...
async def send_photo_or_text(target, image_path: str, caption: str, reply_markup=None):
    ...
    نکته‌ی گپ: gap_types.Message.answer_photo مستقیم مسیرِ فایل رو
    می‌گیره (نه یه آبجکتِ FSInputFile مثل تلگرام).
    """
    if image_path and os.path.isfile(image_path):
        try:
            await target.answer_photo(image_path, caption=caption, reply_markup=reply_markup)
            return
        except Exception:
            pass
...
def home_button() -> list[InlineKeyboardButton]:
    return [InlineKeyboardButton(text="🏠 پنل اصلی", callback_data="menu:home")]

# نکته‌ی گپ: gap_types.InlineKeyboardButton رنگ/استایل نداره، پس
# _ZONE_STYLE حذف شد؛ خطرِ منطقه همچنان با ایموجیِ zone (🟢🟡🔴) تو متنِ
# دکمه معلومه، فقط رنگِ خودِ دکمه دیگه فرق نمی‌کنه.
    ...
    for i, (name, data) in enumerate(MAPS_DATA.items()):
        ...
        buttons.append([
            InlineKeyboardButton(
                text=f"{data['emoji']} {name} ({t_txt}) {zone}",
...
def location_kb(map_idx: int, map_name: str, player: dict | None = None) -> InlineKeyboardMarkup:
    ...
    for li, loc in enumerate(locs):
        if player is not None and not is_explored(player, map_name, li):
            ...
            if loc['name'] in hot:
                label += " 🔥"
        buttons.append([InlineKeyboardButton(text=label, callback_data=f"loc:{map_idx}:{li}")])
    buttons.append([InlineKeyboardButton(text="👑 چالش باس منطقه", callback_data=f"bossch:{map_idx}")])
    buttons.append([InlineKeyboardButton(text="🗣️ دیدار با پادشاهِ شهر", callback_data=f"king:open:{map_name}")])
    buttons.append([InlineKeyboardButton(text="🏮 بازارِ محلی", callback_data=f"cmkt:hub:{map_name}")])
    buttons.append([InlineKeyboardButton(text="🔙 برگشت به نقشه", callback_data="loot:again")])
    buttons.append(home_button())
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
async def cb_boss_challenge(cb: CallbackQuery):
    ...
    if not player:
        ...
        return
    from gap_mob_combat import MIN_HP_PCT_FOR_BOSS_CHALLENGE, start_encounter
    ...
    log_event(map_name, cb.from_user.first_name, "boss", map_name, actor_id=uid)
...
def bm_main_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🛒 فروشگاه",         callback_data="bm:shop")],
        [InlineKeyboardButton(text="🎁 باکس شانسی",       callback_data="bm:lootbox")],
        [InlineKeyboardButton(text="🤝 اعتماد بازار",     callback_data="bm:favor")],
        [InlineKeyboardButton(text="🕵️ تجهیزات جاسوسی", callback_data="bm:spy")],
        [InlineKeyboardButton(text="⚔️ Katana Forge",    callback_data="bm:katana")],
        [InlineKeyboardButton(text="🏰 دفاع پایگاه",     callback_data="bm:defense")],
        [InlineKeyboardButton(text="💎 حراجی سایه",      callback_data="bm:auction")],
        [InlineKeyboardButton(text="🗝️ صندوق‌ها و کلیدها", callback_data="bm:vault")],
        [InlineKeyboardButton(text="🧩 مجموعه‌های ست",    callback_data="bm:sets")],
        [InlineKeyboardButton(text="💰 فروش آیتم",       callback_data="bm:sell")],
        [InlineKeyboardButton(text="📊 وضعیت بازار",     callback_data="bm:market")],
        home_button(),
    ])
...
# ─── Travel ──────────────────────────────────────────────────
async def cb_loot_go(cb: CallbackQuery):
    uid = cb.from_user.id
    try:
        idx = int(cb.data[3:])
    except:
    ...
    await cb.answer(f"🚶 عازم {map_name}!", show_alert=False)
    ...
    sent = await cb.message.answer(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="❌ لغو سفر", callback_data="loot:cancel")
        ]])
    )

    asyncio.create_task(_travel_task(sent, uid, map_name, travel, zone_warn, token))

async def _travel_task(msg, uid: int, map_name: str, travel: int, zone_warn: str, token: int):
    data = MAPS_DATA[map_name]
    elapsed = 0
    interval = 5
...
# ─── Do Loot ─────────────────────────────────────────────────
async def _do_loot(msg: Message, uid: int, map_name: str):
    player = await aget_player(uid)
    ...
    from gap_combat_handlers import update_quest
    update_quest(uid, "loot", 1)

    from gap_mob_combat import start_encounter
    await start_encounter(msg, uid, map_name)

async def cb_loot_location(cb: CallbackQuery):
    uid = cb.from_user.id
    try:
    ...
    if grad_hint:
        await asave_player(uid, player)
        # نکته‌ی گپ: main_kb/_is_group_chat مخصوصِ کیبوردِ ری‌پلایِ تلگرامه؛
        # رو گپ به‌جاش منوی اصلیِ گپ رو نشون می‌دیم.
        from gap_core_handlers import main_menu_kb
    ...
    try:
        ...
        pass
    await _do_loot(cb.message, uid, map_name)
...
async def cb_loot_sell_all(cb: CallbackQuery):
    ...
    tax = await bridge.economy_engine.compute_sell_tax(player, gross_total, zone, "global_loot")
...
async def cb_bm_shop(cb: CallbackQuery):
    ...
    spawned = await bridge.economy_engine.maybe_spawn_random_event("blackmarket_shop")
    ...
    for ev in await bridge.economy_engine.get_active_events_display():
        lines.append(f"🔔 {ev}\n")
    lines.append("\n")
    ...
    for i, item in enumerate(items):
        ...
        buy_p, _, arrow, _ = await bridge.economy_engine.get_dynamic_price("blackmarket_shop", item)
        ...
        buttons.append([InlineKeyboardButton(
            ...
            callback_data=f"bm_buy:{i}")])
    lines.append(f"\n💰 موجودی: **{bz_to_display(zen)}**")
    buttons.append([InlineKeyboardButton(text="🔙 برگشت", callback_data="bm:back")])
    buttons.append(home_button())
...
async def _render_favor_panel(cb: CallbackQuery, player: dict):
    ...
    if is_market_quest_ready(player):
        buttons.append([InlineKeyboardButton(text="🎁 دریافتِ نشانِ اعتماد", callback_data="bm_favor:claim")])
    ...
    for iid, item in MARKET_SPECIAL_ITEMS.items():
        ...
        buttons.append([InlineKeyboardButton(
            ...
            callback_data=f"bm_favor:buy:{iid}")])
...
# ─── BM Katana ───────────────────────────────────────────────
def _render_katana_forge(player: dict) -> tuple[str, InlineKeyboardMarkup]:
    ...
    if nxt:
        ...
        buttons.append([InlineKeyboardButton(
            ...
            callback_data=f"bm_katana_up:{lv}" if can else "bm:noop")])
    else:
        lines.append("\n✅ **حداکثر سطح رسیدی!**")
    buttons.append([InlineKeyboardButton(text="🔙 برگشت", callback_data="bm:back")])
    buttons.append(home_button())
...
async def cb_bm_katana(cb: CallbackQuery):
    ...
    if not player:
        await cb.answer("❌", show_alert=True)
        return
    text, kb = _render_katana_forge(player)
    await bm_render(cb.message, text, reply_markup=kb)
    await cb.answer()


async def cmd_forge(msg: Message):
    """میانبرِ مستقیم: /forge — بدونِ رفتن به منوی اصلیِ بازار سیاه، صاف می‌ره سراغِ Katana Forge."""
    uid = msg.from_user.id
    player = await aget_player(uid)
    if not player:
        await msg.answer("❌ اول /start بزن!")
    ...
    for i, item in enumerate(SHADOW_AUCTION):
        ...
        if item["cost"]:
            buttons.append([InlineKeyboardButton(
                ...
                callback_data=f"bm_auction_buy:{i}")])
...
async def cb_bm_market_overview(cb: CallbackQuery):
    ...
    lines.append(f"🏦 صندوق مالیات سراسری: **{bz_to_display(await bridge.economy_engine.get_tax_pool())}**\n")
    lines.append(f"🎫 رپیوتیشن بازار سیاه‌ات: **{rep}/100** (تخفیف مالیات: {disc}٪)\n")
    lines.append("\n_رپیوتیشن با هر خرید و فروش کم‌کم بالا می‌ره._")

    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 برگشت", callback_data="bm:back")],
        home_button(),
    ])
...
async def cb_bm_sell(cb: CallbackQuery):
    ...
    for item in sellable[:12]:
        _, sell_p, arrow, _ = await bridge.economy_engine.get_dynamic_price("global_loot", item)
        gross_total += sell_p
    ...
    tax = await bridge.economy_engine.compute_sell_tax(player, gross_total, "safe", "blackmarket_shop")
    disc = int(get_reputation_discount(player) * 100)
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"💰 فروش همه (~{bz_to_display(tax['net'])})", callback_data="bm_sell_all")],
        [InlineKeyboardButton(text="🔙 برگشت", callback_data="bm:back")],
        home_button(),
    ])
    await bm_render(cb.message, "".join(lines), reply_markup=kb)
...
async def cb_bm_sell_all(cb: CallbackQuery):
    ...
    await bm_render(
        ...
        f"دریافتی خالص: **{bz_to_display(tax['net'])}**\n"
        f"موجودی: **{bz_to_display(player['zen'])}**",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 برگشت", callback_data="bm:back")], home_button()])
    )
...
async def cb_bm_vault(cb: CallbackQuery):
    ...
    if not boxes:
        ...
        for i, it in boxes[:8]:
            ...
            if have > 0:
                buttons.append([InlineKeyboardButton(text=f"🔓 باز کردن {it['name']}", callback_data=f"lbx_open:{i}")])
        lines.append("\n")

    lines.append("🔑 **خرید کلید:**\n")
    for kid, k in KEYS.items():
        lines.append(f"{k['emoji']} {k['name']} — {bz_to_display(k['buy_price'])}\n")
        buttons.append([InlineKeyboardButton(text=f"خرید {k['emoji']} {k['name']}", callback_data=f"buy_key:{kid}")])
    ...
    buttons.append([InlineKeyboardButton(text=f"🍀 خرید طلسم شانس ({bz_to_display(FORTUNE_WARD_PRICE)})", callback_data="buy_ward")])
...
async def cb_lbx_open(cb: CallbackQuery):
    ...
    await bm_render(cb.message, "".join(lines), reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 برگشت به صندوق‌ها", callback_data="bm:vault")],
        home_button(),
    ]))
...
# ─── Register (نسخه‌ی گپ) ──────────────────────────────────────
def register_gap_loot_handlers(dp: GapDispatcher):
    from gap_raid_handlers import register_gap_raid_handlers
    register_gap_raid_handlers(dp)

    from gap_mob_combat import register_gap_mob_combat_handlers
    register_gap_mob_combat_handlers(dp)

    from gap_lootbox_handlers import register_gap_lootbox_handlers
    register_gap_lootbox_handlers(dp)

    dp.register_message(cmd_loot,        commands=["loot"])
    dp.register_message(cmd_blackmarket, commands=["blackmarket"])
    dp.register_message(cmd_forge,       commands=["forge"])

    # 🆕 میان‌بُرِ کلمه‌ای — فقط توی گروه کار می‌کنه (نگاه کن به gap_group_gate.py)
    from gap_group_gate import group_word
    dp.register_message(group_word(cmd_loot, "/loot"), text="لوت")

    dp.register_callback(cb_loot_go,       data_startswith="lg:")
    dp.register_callback(cb_loot_location, data_startswith="loc:")
    dp.register_callback(cb_boss_challenge, data_startswith="bossch:")

    dp.register_callback(cb_loot_cancel,   data="loot:cancel")
    dp.register_callback(cb_loot_sell_all, data="loot:sell_all")
    dp.register_callback(cb_loot_keep,     data="loot:keep")
    dp.register_callback(cb_loot_again,    data="loot:again")
    dp.register_callback(cb_loot_gamble,   data_startswith="loot:gamble:")

    dp.register_callback(cb_bm_back,        data="bm:back")
    dp.register_callback(cb_bm_shop,        data="bm:shop")
    dp.register_callback(cb_bm_spy,         data="bm:spy")
    dp.register_callback(cb_bm_favor,       data="bm:favor")
    dp.register_callback(cb_bm_katana,      data="bm:katana")
    dp.register_callback(cb_bm_defense,     data="bm:defense")
    dp.register_callback(cb_bm_auction,     data="bm:auction")
    dp.register_callback(cb_bm_sell,        data="bm:sell")
    dp.register_callback(cb_bm_market_overview, data="bm:market")
    dp.register_callback(cb_bm_vault,       data="bm:vault")
    dp.register_callback(cb_bm_sets,        data="bm:sets")
    dp.register_callback(cb_noop,           data="bm:noop")
    dp.register_callback(cb_menu_home,      data="menu:home")

    dp.register_callback(cb_bm_buy,         data_startswith="bm_buy:")
    dp.register_callback(cb_bm_spy_buy,     data_startswith="bm_spy:")
    dp.register_callback(cb_bm_favor_action, data_startswith="bm_favor:")
    dp.register_callback(cb_bm_katana_up,   data_startswith="bm_katana_up:")
    dp.register_callback(cb_bm_def_buy,     data_startswith="bm_def:")
    dp.register_callback(cb_bm_auction_buy, data_startswith="bm_auction_buy:")
    dp.register_callback(cb_bm_sell_all,    data="bm_sell_all")
    dp.register_callback(cb_lbx_open,       data_startswith="lbx_open:")
    dp.register_callback(cb_buy_key,        data_startswith="buy_key:")
    dp.register_callback(cb_buy_ward,       data="buy_ward")
